...
is
   ...
   function Field_Condition (Ctx : Context; Source_Field, Target_Field : Virtual_Field) return Boolean is
     ((case Source_Field is
         ...
         when F_Type_Length_TPID =>
            (case Target_Field is
                  when F_TPID =>
                     RFLX.Types.Bit_Length (Ctx.Cursors (F_Type_Length_TPID).Value.Type_Length_TPID_Value) = 33024,
                  when F_Type_Length =>
                     RFLX.Types.Bit_Length (Ctx.Cursors (F_Type_Length_TPID).Value.Type_Length_TPID_Value) /= 33024,
                  when others =>
                     False),
   ...
   begin
      return ((case Fld is
            ...
            when F_Type_Length_TPID =>
               (Fld => F_Type_Length_TPID, Type_Length_TPID_Value => Convert (Ctx.Buffer.all (Buffer_First .. Buffer_Last), Offset)),
            when F_TPID =>
               (Fld => F_TPID, TPID_Value => Convert (Ctx.Buffer.all (Buffer_First .. Buffer_Last), Offset)),
            when F_TCI =>
               (Fld => F_TCI, TCI_Value => Convert (Ctx.Buffer.all (Buffer_First .. Buffer_Last), Offset)),
            when F_Type_Length =>
               (Fld => F_Type_Length, Type_Length_Value => Convert (Ctx.Buffer.all (Buffer_First .. Buffer_Last), Offset)),
            when F_Payload =>
               (Fld => F_Payload)));
   end Get_Field_Value;
   ...
   function Structural_Valid_Message (Ctx : Context) return Boolean is
     (Valid (Ctx, F_Destination)
      ...
      and then ((Valid (Ctx, F_TPID)
          and then RFLX.Types.Bit_Length (Ctx.Cursors (F_Type_Length_TPID).Value.Type_Length_TPID_Value) = 33024
