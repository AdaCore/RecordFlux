...
is
   ...
   type All_Field_Type is (F_Initial, F_Length, F_Messages, F_Final);

   subtype Field_Type is All_Field_Type range F_Length .. F_Messages;
   ...
   procedure Take_Buffer (Context : in out Context_Type; Buffer : out RFLX.Types.Bytes_Ptr) with
     ...
     Post =>
       Valid_Context (Context)
          ...
          and then Present (Context, F_Messages) = Present (Context, F_Messages)'Old;
   ...
   procedure Verify (Context : in out Context_Type; Field : Field_Type) with
     ...
     Post =>
       Valid_Context (Context)
          ...
          and then (if Field /= F_Messages then (if Valid (Context, F_Messages)'Old then Valid (Context, F_Messages)))
   ...
   procedure Get_Messages (Context : Context_Type) with
     Pre =>
       Valid_Context (Context)
          and then Has_Buffer (Context)
          and then Present (Context, F_Messages);

   procedure Switch (Context : in out Context_Type; Sequence_Context : out Inner_Messages_Sequence.Context_Type) with
     Pre =>
       Valid_Context (Context)
   ...
   procedure Update (Context : in out Context_Type; Sequence_Context : in out Inner_Messages_Sequence.Context_Type) with
     Pre =>
       Valid_Context (Context)
          and then not Has_Buffer (Context)
          and then Present (Context, F_Messages)
