...
is
   ...
   begin
      ...
      case Value.Fld is
         when F_Initial =>
            null;
         when F_Tag =>
            Insert (Value.Tag_Value, Ctx.Buffer.all (Buffer_First .. Buffer_Last), Offset);
         when F_Length =>
   ...
   procedure Set_Tag (Ctx : in out Context; Value : Tag) is
      Field_Value : constant Field_Dependent_Value := (F_Tag, Convert (Value));
