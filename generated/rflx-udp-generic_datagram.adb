...
is
   ...
   function Preliminary_Valid_Predecessors (Context : Context_Type; Field : All_Field_Type) return Boolean is
     ((case Field is
         when F_Initial | F_Source_Port =>
            True,
         when F_Destination_Port =>
            Preliminary_Valid (Context, F_Source_Port),
         when F_Length =>
            Preliminary_Valid (Context, F_Source_Port)
               and then Preliminary_Valid (Context, F_Destination_Port),
         when F_Checksum =>
            Preliminary_Valid (Context, F_Source_Port)
               and then Preliminary_Valid (Context, F_Destination_Port)
         ...
         when F_Payload =>
            Preliminary_Valid (Context, F_Source_Port)
               and then Preliminary_Valid (Context, F_Destination_Port)
               and then Preliminary_Valid (Context, F_Length)
               and then Preliminary_Valid (Context, F_Checksum),
         when F_Final =>
            Preliminary_Valid (Context, F_Source_Port)
               and then Preliminary_Valid (Context, F_Destination_Port)
               and then Preliminary_Valid (Context, F_Length)
   ...
   function Valid_Predecessors (Context : Context_Type; Field : Field_Type) return Boolean is
     ((case Field is
         when F_Source_Port =>
            True,
         when F_Destination_Port =>
            Present (Context, F_Source_Port),
         when F_Length =>
            Present (Context, F_Source_Port)
               and then Present (Context, F_Destination_Port),
         when F_Checksum =>
            Present (Context, F_Source_Port)
               and then Present (Context, F_Destination_Port)
         ...
         when F_Payload =>
            Present (Context, F_Source_Port)
               and then Present (Context, F_Destination_Port)
               and then Present (Context, F_Length)
               and then Present (Context, F_Checksum)))
    with
     Post =>
   ...
   function Valid_Target (Source_Field, Target_Field : All_Field_Type) return Boolean is
     ((case Source_Field is
         when F_Initial =>
            Target_Field = F_Source_Port,
         when F_Source_Port =>
            Target_Field = F_Destination_Port,
         when F_Destination_Port =>
            Target_Field = F_Length,
         when F_Length =>
            Target_Field = F_Checksum,
         when F_Checksum =>
   ...
   function Field_Length (Context : Context_Type; Field : Field_Type) return RFLX.Types.Bit_Length_Type is
     ((case Context.Field is
         ...
         when F_Checksum =>
            (case Field is
                  when F_Payload =>
                     ((RFLX.Types.Bit_Length_Type (Context.Cursors (F_Length).Value.Length_Value) - 8)) * 8,
                  when others =>
                     RFLX.Types.Unreachable_Bit_Length_Type),
   ...
   function Field_First (Context : Context_Type; Field : Field_Type) return RFLX.Types.Bit_Index_Type is
     ((case Context.Field is
         when F_Initial | F_Source_Port | F_Destination_Port | F_Length | F_Checksum | F_Payload | F_Final =>
            Context.Index))
    with
     Pre =>
       Valid_Target (Context.Field, Field)
   ...
   function Field_Postcondition (Context : Context_Type; Field : Field_Type) return Boolean is
     ((case Field is
         when F_Source_Port =>
            Field_Condition (Context, Field, F_Destination_Port),
         when F_Destination_Port =>
            Field_Condition (Context, Field, F_Length),
         when F_Length =>
            Field_Condition (Context, Field, F_Checksum),
         when F_Checksum =>
   ...
   begin
      return ((case Field is
            when F_Source_Port =>
               (Field => F_Source_Port, Source_Port_Value => Convert (Context.Buffer.all (Buffer_First .. Buffer_Last), Offset)),
            when F_Destination_Port =>
               (Field => F_Destination_Port, Destination_Port_Value => Convert (Context.Buffer.all (Buffer_First .. Buffer_Last), Offset)),
            when F_Length =>
               (Field => F_Length, Length_Value => Convert (Context.Buffer.all (Buffer_First .. Buffer_Last), Offset)),
            when F_Checksum =>
               (Field => F_Checksum, Checksum_Value => Convert (Context.Buffer.all (Buffer_First .. Buffer_Last), Offset)),
            when F_Payload =>
               (Field => F_Payload)));
   end Get_Field_Value;
   ...
   begin
      Verify (Context, F_Source_Port);
      Verify (Context, F_Destination_Port);
      Verify (Context, F_Length);
      Verify (Context, F_Checksum);
   ...
   function Incomplete_Message (Context : Context_Type) return Boolean is
     (Incomplete (Context, F_Source_Port)
      or Incomplete (Context, F_Destination_Port)
      or Incomplete (Context, F_Length)
      or Incomplete (Context, F_Checksum)
      or Incomplete (Context, F_Payload));

   function Get_Source_Port (Context : Context_Type) return Port_Type is
     (Context.Cursors (F_Source_Port).Value.Source_Port_Value);

   function Get_Destination_Port (Context : Context_Type) return Port_Type is
     (Context.Cursors (F_Destination_Port).Value.Destination_Port_Value);

   function Get_Length (Context : Context_Type) return Length_Type is
     (Context.Cursors (F_Length).Value.Length_Value);

   function Get_Checksum (Context : Context_Type) return Checksum_Type is
     (Context.Cursors (F_Checksum).Value.Checksum_Value);
