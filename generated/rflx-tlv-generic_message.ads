...
is
   ...
   type All_Field_Type is (F_Initial, F_Tag, F_Length, F_Value, F_Final);

   subtype Field_Type is All_Field_Type range F_Tag .. F_Value;
   ...
   procedure Take_Buffer (Context : in out Context_Type; Buffer : out RFLX.Types.Bytes_Ptr) with
     ...
     Post =>
       Valid_Context (Context)
          ...
          and then Present (Context, F_Tag) = Present (Context, F_Tag)'Old
          and then Present (Context, F_Length) = Present (Context, F_Length)'Old
          and then Present (Context, F_Value) = Present (Context, F_Value)'Old;
   ...
   procedure Verify (Context : in out Context_Type; Field : Field_Type) with
     ...
     Post =>
       Valid_Context (Context)
          and then (if Field /= F_Tag then (if Valid (Context, F_Tag)'Old then Valid (Context, F_Tag)))
          and then (if Field /= F_Length then (if Valid (Context, F_Length)'Old then Valid (Context, F_Length)))
          and then (if Field /= F_Value then (if Valid (Context, F_Value)'Old then Valid (Context, F_Value)))
   ...
   function Get_Tag (Context : Context_Type) return Tag_Type with
     Pre =>
       Valid_Context (Context)
          and then Valid (Context, F_Tag);
   ...
   procedure Get_Value (Context : Context_Type) with
     Pre =>
       Valid_Context (Context)
          and then Has_Buffer (Context)
          and then Present (Context, F_Value);
      ...
      and then (case Field is
           ...
           when F_Tag =>
              (Cursors (F_Tag).State = S_Valid
                 ...
                 and then (Cursors (F_Tag).Last - Cursors (F_Tag).First + 1) = Tag_Type_Base'Size,
           when F_Length =>
              (Cursors (F_Tag).State = S_Valid
                   or Cursors (F_Tag).State = S_Structural_Valid)
