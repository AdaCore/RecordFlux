     ...
     ((case Fld is
         ...
         when F_Final =>
            (Valid (Ctx.Cursors (F_Priority))
                 and then Ctx.Cursors (Fld).Predecessor = F_Priority)));
   ...
   procedure Reset_Dependent_Fields (Ctx : in out Context; Fld : Field) with
     ...
     Post =>
       Valid_Predecessor (Ctx, Fld)
          ...
          and then (if Structural_Valid (Ctx.Cursors (F_Priority)) then
             Ctx.Cursors (F_Priority) = Ctx.Cursors (F_Priority)'Old)
   is
         ...
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            if Valid_Value (Value)
               ...
               pragma Assert ((if Structural_Valid (Ctx.Cursors (F_Priority)) then
                   (Ctx.Cursors (F_Priority).Last - Ctx.Cursors (F_Priority).First + 1) = Priority_Base'Size
