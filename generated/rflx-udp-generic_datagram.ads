...
is
   ...
   procedure Take_Buffer (Ctx : in out Context; Buffer : out RFLX.Types.Bytes_Ptr) with
     ...
     Post =>
       Valid_Context (Ctx)
          ...
          and then Present (Ctx, F_Source_Port) = Present (Ctx, F_Source_Port)'Old
          and then Present (Ctx, F_Destination_Port) = Present (Ctx, F_Destination_Port)'Old
          and then Present (Ctx, F_Length) = Present (Ctx, F_Length)'Old
          and then Present (Ctx, F_Checksum) = Present (Ctx, F_Checksum)'Old
   ...
   procedure Verify (Ctx : in out Context; Fld : Field) with
     ...
     Post =>
       Valid_Context (Ctx)
          and then (if Fld /= F_Source_Port then (if Valid (Ctx, F_Source_Port)'Old then Valid (Ctx, F_Source_Port)))
          and then (if Fld /= F_Destination_Port then (if Valid (Ctx, F_Destination_Port)'Old then Valid (Ctx, F_Destination_Port)))
          and then (if Fld /= F_Length then (if Valid (Ctx, F_Length)'Old then Valid (Ctx, F_Length)))
          and then (if Fld /= F_Checksum then (if Valid (Ctx, F_Checksum)'Old then Valid (Ctx, F_Checksum)))
      ...
      and then (case Fld is
           ...
           when F_Source_Port =>
              (Cursors (F_Source_Port).State = S_Valid
                 ...
                 and then (Cursors (F_Source_Port).Last - Cursors (F_Source_Port).First + 1) = Port'Size,
           when F_Destination_Port =>
              (Cursors (F_Source_Port).State = S_Valid
                   or Cursors (F_Source_Port).State = S_Structural_Valid)
                 ...
                 and then (Cursors (F_Source_Port).Last - Cursors (F_Source_Port).First + 1) = Port'Size
                 and then (Cursors (F_Destination_Port).Last - Cursors (F_Destination_Port).First + 1) = Port'Size,
           when F_Length =>
              (Cursors (F_Source_Port).State = S_Valid
                   or Cursors (F_Source_Port).State = S_Structural_Valid)
                 and then (Cursors (F_Destination_Port).State = S_Valid
                 ...
                 and then (Cursors (F_Source_Port).Last - Cursors (F_Source_Port).First + 1) = Port'Size
                 and then (Cursors (F_Destination_Port).Last - Cursors (F_Destination_Port).First + 1) = Port'Size
                 and then (Cursors (F_Length).Last - Cursors (F_Length).First + 1) = Length_Base'Size,
           when F_Checksum =>
              (Cursors (F_Source_Port).State = S_Valid
                   or Cursors (F_Source_Port).State = S_Structural_Valid)
                 and then (Cursors (F_Destination_Port).State = S_Valid
                 ...
                 and then (Cursors (F_Destination_Port).Last - Cursors (F_Destination_Port).First + 1) = Port'Size
                 and then (Cursors (F_Length).Last - Cursors (F_Length).First + 1) = Length_Base'Size
                 and then (Cursors (F_Checksum).Last - Cursors (F_Checksum).First + 1) = Checksum'Size,
