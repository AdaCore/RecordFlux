...
is
   ...
   type All_Field_Type is (F_Initial, F_Source_Port, F_Destination_Port, F_Length, F_Checksum, F_Payload, F_Final);

   subtype Field_Type is All_Field_Type range F_Source_Port .. F_Payload;
   ...
   procedure Take_Buffer (Context : in out Context_Type; Buffer : out RFLX.Types.Bytes_Ptr) with
     ...
     Post =>
       Valid_Context (Context)
          ...
          and then Present (Context, F_Source_Port) = Present (Context, F_Source_Port)'Old
          and then Present (Context, F_Destination_Port) = Present (Context, F_Destination_Port)'Old
          and then Present (Context, F_Length) = Present (Context, F_Length)'Old
          and then Present (Context, F_Checksum) = Present (Context, F_Checksum)'Old
   ...
   procedure Verify (Context : in out Context_Type; Field : Field_Type) with
     ...
     Post =>
       Valid_Context (Context)
          and then (if Field /= F_Source_Port then (if Valid (Context, F_Source_Port)'Old then Valid (Context, F_Source_Port)))
          and then (if Field /= F_Destination_Port then (if Valid (Context, F_Destination_Port)'Old then Valid (Context, F_Destination_Port)))
          and then (if Field /= F_Length then (if Valid (Context, F_Length)'Old then Valid (Context, F_Length)))
          and then (if Field /= F_Checksum then (if Valid (Context, F_Checksum)'Old then Valid (Context, F_Checksum)))
   ...
   function Get_Source_Port (Context : Context_Type) return Port_Type with
     Pre =>
       Valid_Context (Context)
          and then Valid (Context, F_Source_Port);

   function Get_Destination_Port (Context : Context_Type) return Port_Type with
     Pre =>
       Valid_Context (Context)
          and then Valid (Context, F_Destination_Port);
   ...
   function Get_Checksum (Context : Context_Type) return Checksum_Type with
     Pre =>
       Valid_Context (Context)
          and then Valid (Context, F_Checksum);
      ...
      record
         case Field is
            when F_Initial | F_Payload | F_Final =>
               null;
            when F_Source_Port =>
               Source_Port_Value : Port_Type;
            when F_Destination_Port =>
               Destination_Port_Value : Port_Type;
            when F_Length =>
               Length_Value : Length_Type_Base;
            when F_Checksum =>
     ...
     ((if Buffer /= null then Buffer'First = Buffer_First
      ...
      and then (case Field is
           when F_Initial =>
              True,
           when F_Source_Port =>
              (Cursors (F_Source_Port).State = S_Valid
                   or Cursors (F_Source_Port).State = S_Structural_Valid)
                 and then (Cursors (F_Source_Port).Last - Cursors (F_Source_Port).First + 1) = Port_Type'Size,
           when F_Destination_Port =>
              (Cursors (F_Source_Port).State = S_Valid
                   or Cursors (F_Source_Port).State = S_Structural_Valid)
                 and then (Cursors (F_Destination_Port).State = S_Valid
                   or Cursors (F_Destination_Port).State = S_Structural_Valid)
                 and then (Cursors (F_Source_Port).Last - Cursors (F_Source_Port).First + 1) = Port_Type'Size
                 and then (Cursors (F_Destination_Port).Last - Cursors (F_Destination_Port).First + 1) = Port_Type'Size,
           when F_Length =>
              (Cursors (F_Source_Port).State = S_Valid
                   or Cursors (F_Source_Port).State = S_Structural_Valid)
                 and then (Cursors (F_Destination_Port).State = S_Valid
                 ...
                 and then (Cursors (F_Source_Port).Last - Cursors (F_Source_Port).First + 1) = Port_Type'Size
                 and then (Cursors (F_Destination_Port).Last - Cursors (F_Destination_Port).First + 1) = Port_Type'Size
                 and then (Cursors (F_Length).Last - Cursors (F_Length).First + 1) = Length_Type_Base'Size,
           when F_Checksum =>
              (Cursors (F_Source_Port).State = S_Valid
                   or Cursors (F_Source_Port).State = S_Structural_Valid)
                 and then (Cursors (F_Destination_Port).State = S_Valid
                 ...
                 and then (Cursors (F_Length).State = S_Valid
                   or Cursors (F_Length).State = S_Structural_Valid)
                 and then (Cursors (F_Checksum).State = S_Valid
                   or Cursors (F_Checksum).State = S_Structural_Valid)
                 and then (Cursors (F_Source_Port).Last - Cursors (F_Source_Port).First + 1) = Port_Type'Size
                 and then (Cursors (F_Destination_Port).Last - Cursors (F_Destination_Port).First + 1) = Port_Type'Size
                 and then (Cursors (F_Length).Last - Cursors (F_Length).First + 1) = Length_Type_Base'Size
                 and then (Cursors (F_Checksum).Last - Cursors (F_Checksum).First + 1) = Checksum_Type'Size,
