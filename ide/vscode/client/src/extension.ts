...
import { registerLogger, traceLog, traceError, traceVerbose } from "./common/log/logging";
...
function setupLogging(context: vscode.ExtensionContext, serverInfo: IServerInfo): vscode.LogOutputChannel {
...
async function runServer(serverInfo: IServerInfo, outputChannel: vscode.LogOutputChannel): Promise<void> {
	const interpreter = getInterpreterFromSetting(serverInfo.module);
	...
	if (!await isServerInstalled(serverInfo.module)) {
		...
		const workspaceSetting = await getWorkspaceSettings(serverInfo.module, projectRoot, true);

		const result = await vscode.window.showWarningMessage(
			"RecordFlux language server is not installed in the selected environment",
			"Install", "Select Environment"
		);
	...
	client = await restartServer(serverInfo.module, serverInfo.name, outputChannel, client);
}
