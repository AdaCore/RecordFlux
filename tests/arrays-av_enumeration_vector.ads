package Arrays.AV_Enumeration_Vector
  with SPARK_Mode
is
   ...
   function Valid_Element (Buffer : Types.Bytes; Cursor : Cursor_Type) return Boolean is
      (((Cursor.First >= Buffer'First and then Cursor.Last <= Buffer'Last) and then (Cursor.First <= Cursor.Last and then Buffer (Cursor.First .. Cursor.Last)'Length = 1)))
     with
       Pre => Is_Contained (Buffer);

   function Get_Element (Buffer : Types.Bytes; Cursor : Cursor_Type) return AV_Enumeration is
      (Convert_To_AV_Enumeration (Buffer (Cursor.First .. Cursor.Last), 0))
     with
       Pre => (Is_Contained (Buffer) and then Valid_Element (Buffer, Cursor));

end Arrays.AV_Enumeration_Vector;
