...
is
   ...
   function Valid_Element (Buffer : Types.Bytes; Cursor : Cursor_Type) return Boolean is
      (((Cursor.First >= Buffer'First and then Cursor.Last <= Buffer'Last) and then (Cursor.First <= Cursor.Last and then Buffer (Cursor.First .. Cursor.Last)'Length = 2)))
     with
       Pre => Is_Contained (Buffer);

   function Get_Element (Buffer : Types.Bytes; Cursor : Cursor_Type) return Modular_Integer is
      (Convert_To_Modular_Integer (Buffer (Cursor.First .. Cursor.Last), 0))
     with
