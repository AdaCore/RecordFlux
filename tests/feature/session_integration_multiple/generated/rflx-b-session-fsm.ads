...
with RFLX.B.Session.FSM_Allocator;
with RFLX.RFLX_Types;
with RFLX.Universal;
with RFLX.Universal.Message;

package RFLX.B.Session.FSM with
  SPARK_Mode
is
...
private
   ...
   type Private_Context is
      record
         ...
         Slots : B.Session.FSM_Allocator.Slots;
         Memory : B.Session.FSM_Allocator.Memory;
      end record;

   function Uninitialized (Ctx : Context) return Boolean is
     (not Universal.Message.Has_Buffer (Ctx.P.M_Ctx)
      and B.Session.FSM_Allocator.Uninitialized (Ctx.P.Slots));
   ...
   function Global_Allocated (Ctx : Context) return Boolean is
     (B.Session.FSM_Allocator.Global_Allocated (Ctx.P.Slots));
