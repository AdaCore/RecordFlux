...
is
   ...
   with
     Pre =>
       ...
       and then RFLX.Test.Message.Sufficient_Buffer_Length (Ctx, Fld) is
   ...
   with
     ...
     Post =>
       ...
       and then (Ctx.Cursors (Fld).Value = Val
                 and then (if Fld in F_Value and then Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, Fld))) is
