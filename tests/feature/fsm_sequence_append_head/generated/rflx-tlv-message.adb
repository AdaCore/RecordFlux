...
is
   ...
   procedure Initialize_Value_Private (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.TLV.Message.Has_Buffer (Ctx)
   ...
   is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Value));
   begin
      if Length > 0 then
         Process_Value (Ctx.Buffer.all (First .. First + RFLX_Types.Index (Length) - 1));
      end if;
      pragma Assert (RFLX.TLV.Message.Valid_Length (Ctx, RFLX.TLV.Message.F_Value, Length));
