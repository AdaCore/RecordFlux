...
is
   ...
   type Context (Buffer_First, Buffer_Last : RFLX_Types.Index := RFLX_Types.Index'First; First : RFLX_Types.Bit_Index := RFLX_Types.Bit_Index'First; Last : RFLX_Types.Bit_Length := RFLX_Types.Bit_Length'First; P : Boolean := Boolean'First) is private
   with
   ...
   procedure Initialize (Ctx : out Context; Buffer : in out RFLX_Types.Bytes_Ptr; P : Boolean; Written_Last : RFLX_Types.Bit_Length := 0)
   with
     Pre =>
       not Ctx'Constrained
   ...
   procedure Initialize (Ctx : out Context; Buffer : in out RFLX_Types.Bytes_Ptr; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; P : Boolean; Written_Last : RFLX_Types.Bit_Length := 0)
   with
     Pre =>
       not Ctx'Constrained
       and then Buffer /= null
   ...
   procedure Reset (Ctx : in out Context; P : Boolean)
   with
     Pre =>
       not Ctx'Constrained
   ...
   procedure Reset (Ctx : in out Context; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; P : Boolean)
   with
     Pre =>
       not Ctx'Constrained
       and RFLX.Test.M.Has_Buffer (Ctx)
   ...
   function Get_F1 (Ctx : Context) return RFLX.Universal.Value
   with
     Pre =>
       RFLX.Test.M.Valid (Ctx, RFLX.Test.M.F_F1);

   function Get_F2 (Ctx : Context) return RFLX.Universal.Value
   with
     Pre =>
   ...
   procedure Set_F1 (Ctx : in out Context; Val : RFLX.Universal.Value)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
   ...
   procedure Set_F2 (Ctx : in out Context; Val : RFLX.Universal.Value)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
