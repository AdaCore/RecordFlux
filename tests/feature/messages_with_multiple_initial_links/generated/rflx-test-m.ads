...
is
   ...
   type Virtual_Field is (F_Initial, F_F1, F_F2, F_Final);

   subtype Field is Virtual_Field range F_F1 .. F_F2;

   type Field_Cursor is private;

   type Field_Cursors is private;

   type Context (Buffer_First, Buffer_Last : RFLX_Types.Index := RFLX_Types.Index'First; First : RFLX_Types.Bit_Index := RFLX_Types.Bit_Index'First; Last : RFLX_Types.Bit_Length := RFLX_Types.Bit_Length'First; P : Boolean := Boolean'First) is private with
   ...
   procedure Initialize (Ctx : out Context; Buffer : in out RFLX_Types.Bytes_Ptr; P : Boolean; Written_Last : RFLX_Types.Bit_Length := 0) with
     Pre =>
       not Ctx'Constrained
     ...
     Post =>
       ...
       and Initialized (Ctx),
     Depends =>
       (Ctx => (Buffer, P, Written_Last), Buffer => null);

   procedure Initialize (Ctx : out Context; Buffer : in out RFLX_Types.Bytes_Ptr; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; P : Boolean; Written_Last : RFLX_Types.Bit_Length := 0) with
     Pre =>
       not Ctx'Constrained
       and then Buffer /= null
       and then Buffer'Length > 0
     ...
     Post =>
       ...
       and Initialized (Ctx),
     Depends =>
       (Ctx => (Buffer, First, Last, P, Written_Last), Buffer => null);
   ...
   procedure Reset (Ctx : in out Context; P : Boolean) with
     Pre =>
       not Ctx'Constrained
       and RFLX.Test.M.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Reset (Ctx : in out Context; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; P : Boolean) with
     Pre =>
       not Ctx'Constrained
       and RFLX.Test.M.Has_Buffer (Ctx)
   ...
   procedure Copy (Ctx : Context; Buffer : out RFLX_Types.Bytes) with
     Pre =>
       RFLX.Test.M.Has_Buffer (Ctx)
       and then RFLX.Test.M.Well_Formed_Message (Ctx)
       and then RFLX.Test.M.Byte_Size (Ctx) = Buffer'Length;
   ...
   procedure Generic_Write (Ctx : in out Context; Offset : RFLX_Types.Length := 0) with
     Pre =>
       ...
       and then Offset < RFLX.Test.M.Buffer_Length (Ctx)
       and then Pre (RFLX.Test.M.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   procedure Data (Ctx : Context; Data : out RFLX_Types.Bytes) with
     Pre =>
       RFLX.Test.M.Has_Buffer (Ctx)
       and then RFLX.Test.M.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.Test.M.Byte_Size (Ctx);
   ...
   function Field_Condition (Ctx : Context; Fld : Field) return Boolean with
     Pre =>
       ...
       and then RFLX.Test.M.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   function Get_F1 (Ctx : Context) return RFLX.Universal.Value with
     Pre =>
       RFLX.Test.M.Valid (Ctx, RFLX.Test.M.F_F1);

   function Get_F2 (Ctx : Context) return RFLX.Universal.Value with
     Pre =>
       RFLX.Test.M.Valid (Ctx, RFLX.Test.M.F_F2);
   ...
   procedure Set_F1 (Ctx : in out Context; Val : RFLX.Universal.Value) with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Test.M.Has_Buffer (Ctx)
       and then RFLX.Test.M.Valid_Next (Ctx, RFLX.Test.M.F_F1)
       and then RFLX.Universal.Valid_Value (RFLX.Universal.To_Base_Integer (Val))
       and then RFLX.Test.M.Available_Space (Ctx, RFLX.Test.M.F_F1) >= RFLX.Test.M.Field_Size (Ctx, RFLX.Test.M.F_F1)
       and then RFLX.Test.M.Field_Condition (Ctx, RFLX.Test.M.F_F1),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_F1)
       and Get_F1 (Ctx) = Val
       and Invalid (Ctx, F_F2)
       and Valid_Next (Ctx, F_F2)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Ctx.P = Ctx.P'Old
       and Valid_Next (Ctx, F_F1) = Valid_Next (Ctx, F_F1)'Old
       and Field_First (Ctx, F_F1) = Field_First (Ctx, F_F1)'Old;

   procedure Set_F2 (Ctx : in out Context; Val : RFLX.Universal.Value) with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Test.M.Has_Buffer (Ctx)
       and then RFLX.Test.M.Valid_Next (Ctx, RFLX.Test.M.F_F2)
       and then RFLX.Universal.Valid_Value (RFLX.Universal.To_Base_Integer (Val))
       and then RFLX.Test.M.Available_Space (Ctx, RFLX.Test.M.F_F2) >= RFLX.Test.M.Field_Size (Ctx, RFLX.Test.M.F_F2)
       and then RFLX.Test.M.Field_Condition (Ctx, RFLX.Test.M.F_F2),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_F2)
       and Get_F2 (Ctx) = Val
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_F2))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Ctx.P = Ctx.P'Old
       and Valid_Next (Ctx, F_F2) = Valid_Next (Ctx, F_F2)'Old
       and Field_First (Ctx, F_F2) = Field_First (Ctx, F_F2)'Old
       and (for all F in Field range F_F1 .. F_F1 =>
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; P : Boolean) return Boolean is
     ((if Well_Formed (Cursors (F_F1)) then P)
      and then (if Well_Formed (Cursors (F_F2)) then Valid (Cursors (F_F1)) or not P)) with
     Pre =>
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; P : Boolean; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          ...
          when F_F2 =>
             Field_First_F2 (Cursors, First, Verified_Last, Written_Last, P)) with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last, P)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, P, Fld),
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors; P : Boolean) return Boolean is
     ((if
          Buffer /= null
       then
          Buffer'Length > 0
      ...
      and then ((if
                    Well_Formed (Cursors (F_F1))
                 then
                    (Cursors (F_F1).Last - Cursors (F_F1).First + 1 = 8
                     and then Cursors (F_F1).First = First))
                and then (if
                          ...
                          then
                             (if
                                 ...
                                 and then True
                              then
                                 Cursors (F_F2).Last - Cursors (F_F2).First + 1 = 8
                                 and then Cursors (F_F2).First = Cursors (F_F1).Last + 1)
                             and then (Cursors (F_F2).Last - Cursors (F_F2).First + 1 = 8
                                       and then Cursors (F_F2).First = First)))) with
     Post =>
       True;
   ...
   type Context (Buffer_First, Buffer_Last : RFLX_Types.Index := RFLX_Types.Index'First; First : RFLX_Types.Bit_Index := RFLX_Types.Bit_Index'First; Last : RFLX_Types.Bit_Length := RFLX_Types.Bit_Length'First; P : Boolean := Boolean'First) is
      record
     ...
     Dynamic_Predicate =>
       Valid_Context (Context.Buffer_First, Context.Buffer_Last, Context.First, Context.Last, Context.Verified_Last, Context.Written_Last, Context.Buffer, Context.Cursors, Context.P);

   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      and then Valid_Next (Ctx, F_F1)
      and then RFLX.Test.M.Field_First (Ctx, RFLX.Test.M.F_F1) rem RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_F1) = Ctx.Last - Ctx.First + 1
   ...
   function Get_F1 (Ctx : Context) return RFLX.Universal.Value is
     (To_Actual (Ctx.Cursors (F_F1).Value));

   function Get_F2 (Ctx : Context) return RFLX.Universal.Value is
     (To_Actual (Ctx.Cursors (F_F2).Value));
