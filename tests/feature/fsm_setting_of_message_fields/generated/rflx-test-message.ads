...
is
   ...
   type Virtual_Field is (F_Initial, F_Has_Data, F_Length, F_Data, F_Final);

   subtype Field is Virtual_Field range F_Has_Data .. F_Data;
   ...
   function Field_Condition (Ctx : Context; Fld : Field; Val : RFLX_Types.Base_Integer; Agg : RFLX_Types.Bytes; Size : RFLX_Types.Bit_Length := 0) return Boolean with
     Pre =>
       RFLX.Test.Message.Has_Buffer (Ctx)
   ...
   function Get_Has_Data (Ctx : Context) return Boolean with
     Pre =>
       RFLX.Test.Message.Valid (Ctx, RFLX.Test.Message.F_Has_Data);

   function Get_Length (Ctx : Context) return RFLX.Test.Length with
     Pre =>
       RFLX.Test.Message.Valid (Ctx, RFLX.Test.Message.F_Length);
   ...
   procedure Set_Has_Data (Ctx : in out Context; Val : Boolean) with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
       and then RFLX.Test.Message.Has_Buffer (Ctx)
       and then RFLX.Test.Message.Valid_Next (Ctx, RFLX.Test.Message.F_Has_Data)
       and then Valid_Boolean (To_Base_Integer (Val))
       and then RFLX.Test.Message.Available_Space (Ctx, RFLX.Test.Message.F_Has_Data) >= RFLX.Test.Message.Field_Size (Ctx, RFLX.Test.Message.F_Has_Data)
       and then RFLX.Test.Message.Field_Condition (Ctx, RFLX.Test.Message.F_Has_Data, To_Base_Integer (Val), (1 .. 0 => 0)),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Has_Data)
       and Get_Has_Data (Ctx) = Val
       and Invalid (Ctx, F_Length)
       and Invalid (Ctx, F_Data)
       and (if Get_Has_Data (Ctx) then Valid_Next (Ctx, F_Length))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Has_Data) = Valid_Next (Ctx, F_Has_Data)'Old
       and Field_First (Ctx, F_Has_Data) = Field_First (Ctx, F_Has_Data)'Old;
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.Test.Length) with
     ...
     Pre =>
       not Ctx'Constrained
       and then RFLX.Test.Message.Has_Buffer (Ctx)
       and then RFLX.Test.Message.Valid_Next (Ctx, RFLX.Test.Message.F_Length)
       and then RFLX.Test.Valid_Length (RFLX.Test.To_Base_Integer (Val))
       and then RFLX.Test.Message.Available_Space (Ctx, RFLX.Test.Message.F_Length) >= RFLX.Test.Message.Field_Size (Ctx, RFLX.Test.Message.F_Length)
       and then RFLX.Test.Message.Field_Condition (Ctx, RFLX.Test.Message.F_Length, RFLX.Test.To_Base_Integer (Val), (1 .. 0 => 0)),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Length)
       and Get_Length (Ctx) = Val
       and Invalid (Ctx, F_Data)
       and (if Get_Length (Ctx) > 0 then Valid_Next (Ctx, F_Data))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Valid_Next (Ctx, F_Length) = Valid_Next (Ctx, F_Length)'Old
       and Get_Has_Data (Ctx) = Get_Has_Data (Ctx)'Old
       and Field_First (Ctx, F_Length) = Field_First (Ctx, F_Length)'Old
       and (for all F in Field range F_Has_Data .. F_Has_Data =>
   ...
   procedure Initialize_Data (Ctx : in out Context) with
     ...
     Post =>
       ...
       and then Get_Has_Data (Ctx) = Get_Has_Data (Ctx)'Old
   ...
   procedure Set_Data (Ctx : in out Context; Data : RFLX_Types.Bytes) with
     Pre =>
       ...
       and then RFLX.Test.Message.Field_Condition (Ctx, RFLX.Test.Message.F_Data, 0, Data, RFLX_Types.To_Bit_Length (Data'Length)),
     Post =>
       Has_Buffer (Ctx)
   ...
   type Structure is
      record
         ...
         Data : RFLX_Types.Bytes (RFLX_Types.Index'First .. RFLX_Types.Index'First + 1);
      end record;
   ...
   function Field_Size_Has_Data (Struct : Structure) return RFLX_Types.Bit_Length with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr) return Boolean is
     ((if Well_Formed (Cursors (F_Has_Data)) then True)
      and then (if
                   Well_Formed (Cursors (F_Length))
                then
                   (Valid (Cursors (F_Has_Data))
                    and then To_Actual (Cursors (F_Has_Data).Value)))
      and then (if
                   Well_Formed (Cursors (F_Data))
                then
                   (Valid (Cursors (F_Length))
                    and then Cursors (F_Length).Value > 0))) with
     Pre =>
   ...
   function Field_Size_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Fld : Field) return RFLX_Types.Bit_Length'Base is
     (case Fld is
          ...
          when F_Length =>
             7,
          when F_Data =>
             RFLX_Types.Bit_Length (Cursors (F_Length).Value) * 8) with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last, Buffer)
   ...
   function Field_First_Has_Data (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr) return RFLX_Types.Bit_Index'Base is
     (First) with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last, Buffer)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, Buffer, F_Has_Data);
   ...
   function Field_First_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Fld : Field) return RFLX_Types.Bit_Index'Base is
     (case Fld is
          when F_Has_Data =>
             Field_First_Has_Data (Cursors, First, Verified_Last, Written_Last, Buffer),
          when F_Length =>
             Field_First_Length (Cursors, First, Verified_Last, Written_Last, Buffer),
          when F_Data =>
             Field_First_Data (Cursors, First, Verified_Last, Written_Last, Buffer)) with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
       and then Valid_Predecessors_Invariant (Cursors, First, Verified_Last, Written_Last, Buffer)
       and then Valid_Next_Internal (Cursors, First, Verified_Last, Written_Last, Buffer, Fld),
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if
      ...
      and then ((if
                 ...
                 then
                    (Cursors (F_Has_Data).Last - Cursors (F_Has_Data).First + 1 = 1
                     and then Cursors (F_Has_Data).First = First))
                and then (if
                             Well_Formed (Cursors (F_Length))
                          then
                             (Cursors (F_Length).Last - Cursors (F_Length).First + 1 = 7
                              and then Cursors (F_Length).First = Cursors (F_Has_Data).Last + 1))
                and then (if
                             Well_Formed (Cursors (F_Data))
                          then
                             (Cursors (F_Data).Last - Cursors (F_Data).First + 1 = RFLX_Types.Bit_Length (Cursors (F_Length).Value) * 8
                              and then Cursors (F_Data).First = Cursors (F_Length).Last + 1)))) with
     Post =>
       True;
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      and then Valid_Next (Ctx, F_Has_Data)
      and then RFLX.Test.Message.Field_First (Ctx, RFLX.Test.Message.F_Has_Data) rem RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_Has_Data) = Ctx.Last - Ctx.First + 1
   ...
   function Field_Condition (Ctx : Context; Fld : Field; Val : RFLX_Types.Base_Integer; Agg : RFLX_Types.Bytes; Size : RFLX_Types.Bit_Length := 0) return Boolean is
     (case Fld is
          when F_Has_Data =>
             To_Actual (Val),
          when F_Length =>
          ...
          when F_Data =>
             Agg /= (0, 0)
             and then RFLX_Types.Base_Integer (Size) > 0);
   ...
   function Well_Formed_Message (Ctx : Context) return Boolean is
     (Well_Formed (Ctx, F_Data)
      and then not Equal (Ctx, F_Data, (RFLX_Types.Byte'Val (0), RFLX_Types.Byte'Val (0)))
      and then RFLX_Types.Base_Integer (Ctx.Cursors (F_Data).Last - Ctx.Cursors (F_Data).First + 1) > 0);
   ...
   function Get_Has_Data (Ctx : Context) return Boolean is
     (To_Actual (Ctx.Cursors (F_Has_Data).Value));
   ...
   function Valid_Structure (Struct : Structure) return Boolean is
     (Struct.Has_Data
      and then RFLX_Types.Base_Integer (Struct.Length) > 0
      and then (Struct.Data /= (0, 0)
                and then Struct.Data'Size > 0));
   ...
   function Field_Size_Has_Data (Struct : Structure) return RFLX_Types.Bit_Length is
     (1);
