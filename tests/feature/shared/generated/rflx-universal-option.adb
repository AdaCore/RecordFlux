...
is
   ...
   begin
      Ctx := (Buffer_First, Buffer_Last, First, Last, First - 1, (if Written_Last = 0 then First - 1 else Written_Last), Buffer, (F_Option_Type => (State => S_Invalid, Predecessor => F_Initial, others => <>), others => (State => S_Invalid, Predecessor => F_Final, others => <>)));
      Buffer := null;
   end Initialize;
   ...
   begin
      Ctx := (Ctx.Buffer_First, Ctx.Buffer_Last, First, Last, First - 1, First - 1, Ctx.Buffer, (F_Option_Type => (State => S_Invalid, Predecessor => F_Initial, others => <>), others => (State => S_Invalid, Predecessor => F_Final, others => <>)));
   end Reset;
   ...
   function Successor (Ctx : Context; Fld : Field) return Virtual_Field is
     ((case Fld is
          when F_Option_Type =>
             (if
                 RFLX_Types.Base_Integer (Ctx.Cursors (F_Option_Type).Value) = RFLX_Types.Base_Integer (To_Base_Integer (RFLX.Universal.OT_Null))
              then
                 F_Final
              elsif
   ...
   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer with
     Pre =>
       ...
       and then RFLX.Universal.Option.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.Universal.Option.Composite_Field (Fld)
   is
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       ...
       and then RFLX.Universal.Option.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Universal.Option.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.Universal.Option.Composite_Field (Fld)
                 then
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.Universal.Option.Valid_Next (Ctx, Fld)
       and then Fld in F_Option_Type | F_Length
       and then RFLX.Universal.Option.Valid_Value (Fld, Val)
       and then RFLX.Universal.Option.Valid_Size (Ctx, Fld, RFLX.Universal.Option.Field_Size (Ctx, Fld))
       and then RFLX.Universal.Option.Available_Space (Ctx, Fld) >= RFLX.Universal.Option.Field_Size (Ctx, Fld)
       and then RFLX.Universal.Option.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Universal.Option.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
       and Invalid_Successor (Ctx, Fld)
   ...
   begin
      Set_Scalar (Ctx, F_Option_Type, RFLX.Universal.To_Base_Integer (Val));
   end Set_Option_Type;
