...
is
   ...
   function Path_Condition (Ctx : Context; Fld : Field) return Boolean with
     Pre =>
       RFLX.Messages.Msg.Valid_Predecessor (Ctx, Fld),
     Post =>
       True;
   ...
   function Field_Condition (Ctx : Context; Fld : Field) return Boolean with
     Pre =>
       RFLX.Messages.Msg.Has_Buffer (Ctx)
       and then RFLX.Messages.Msg.Valid_Predecessor (Ctx, Fld)
...
private
   ...
   function Valid_Next_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Fld : Field) return Boolean is
     ((case Fld is
          when F_A =>
             Cursors (F_A).Predecessor = F_Initial,
          when F_B =>
             (Valid (Cursors (F_A))
              and then True
   ...
   function Field_First (Ctx : Context; Fld : Field) return RFLX_Types.Bit_Index is
     ((if Fld = F_A then Ctx.First else Ctx.Cursors (Ctx.Cursors (Fld).Predecessor).Last + 1));
