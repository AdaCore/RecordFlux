...
is
   ...
   procedure Generic_Write (Ctx : in out Context; Offset : RFLX_Types.Length := 0) with
     Pre =>
       ...
       and then Offset < RFLX.Messages.Msg_LE.Buffer_Length (Ctx)
       and then Pre (RFLX.Messages.Msg_LE.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   procedure Data (Ctx : Context; Data : out RFLX_Types.Bytes) with
     Pre =>
       RFLX.Messages.Msg_LE.Has_Buffer (Ctx)
       and then RFLX.Messages.Msg_LE.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.Messages.Msg_LE.Byte_Size (Ctx);
   ...
   function Path_Condition (Ctx : Context; Fld : Field) return Boolean with
     Pre =>
       RFLX.Messages.Msg_LE.Valid_Predecessor (Ctx, Fld),
     Post =>
       True;
   ...
   function Field_Condition (Ctx : Context; Fld : Field) return Boolean with
     Pre =>
       RFLX.Messages.Msg_LE.Has_Buffer (Ctx)
       and then RFLX.Messages.Msg_LE.Valid_Predecessor (Ctx, Fld)
   ...
   procedure Set_C (Ctx : in out Context; Val : RFLX.Messages.Integer) with
     ...
     Pre =>
       not Ctx'Constrained
       and then RFLX.Messages.Msg_LE.Has_Buffer (Ctx)
       and then RFLX.Messages.Msg_LE.Valid_Next (Ctx, RFLX.Messages.Msg_LE.F_C)
       and then RFLX.Messages.Valid_Integer (RFLX.Messages.To_Base_Integer (Val))
       and then RFLX.Messages.Msg_LE.Available_Space (Ctx, RFLX.Messages.Msg_LE.F_C) >= RFLX.Messages.Msg_LE.Field_Size (Ctx, RFLX.Messages.Msg_LE.F_C)
       and then RFLX.Messages.Msg_LE.Field_Condition (Ctx, RFLX.Messages.Msg_LE.F_C),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_C)
       and Get_C (Ctx) = Val
   ...
   procedure Set_D (Ctx : in out Context; Val : RFLX.Messages.Enum_T) with
     ...
     Pre =>
       not Ctx'Constrained
       and then RFLX.Messages.Msg_LE.Has_Buffer (Ctx)
       and then RFLX.Messages.Msg_LE.Valid_Next (Ctx, RFLX.Messages.Msg_LE.F_D)
       and then RFLX.Messages.Valid_Enum_T (RFLX.Messages.To_Base_Integer (Val))
       and then RFLX.Messages.Msg_LE.Available_Space (Ctx, RFLX.Messages.Msg_LE.F_D) >= RFLX.Messages.Msg_LE.Field_Size (Ctx, RFLX.Messages.Msg_LE.F_D)
       and then RFLX.Messages.Msg_LE.Field_Condition (Ctx, RFLX.Messages.Msg_LE.F_D),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_D)
       and Get_D (Ctx) = Val
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_D))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       ...
       and Field_First (Ctx, F_D) = Field_First (Ctx, F_D)'Old
       and Field_Last (Ctx, F_D) = Field_Last (Ctx, Predecessor (Ctx, F_D)) + Field_Size (Ctx, F_D)
   ...
   procedure To_Context (Struct : Structure; Ctx : in out Context) with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Messages.Msg_LE.Has_Buffer (Ctx)
       and then RFLX.Messages.Msg_LE.Valid_Structure (Struct)
       and then RFLX.Messages.Msg_LE.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
...
private
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.Messages.Msg_LE.Field_First (Ctx, RFLX.Messages.Msg_LE.F_C) rem RFLX_Types.Byte'Size = 1
   ...
   function Get_C (Ctx : Context) return RFLX.Messages.Integer is
     (To_Actual (Ctx.Cursors (F_C).Value));
   ...
   function Field_Size_C (Struct : Structure) return RFLX_Types.Bit_Length is
     (32);

   function Field_Size_D (Struct : Structure) return RFLX_Types.Bit_Length is
     (32);
