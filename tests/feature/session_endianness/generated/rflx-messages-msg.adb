...
is
   ...
   begin
      Ctx := (Buffer_First, Buffer_Last, First, Last, First - 1, (if Written_Last = 0 then First - 1 else Written_Last), Buffer, (F_A => (State => S_Invalid, Predecessor => F_Initial, others => <>), others => (State => S_Invalid, Predecessor => F_Final, others => <>)));
      Buffer := null;
   end Initialize;
   ...
   begin
      Ctx := (Ctx.Buffer_First, Ctx.Buffer_Last, First, Last, First - 1, First - 1, Ctx.Buffer, (F_A => (State => S_Invalid, Predecessor => F_Initial, others => <>), others => (State => S_Invalid, Predecessor => F_Final, others => <>)));
   end Reset;
   ...
   function Successor (Ctx : Context; Fld : Field) return Virtual_Field is
    ...
    with
     Pre =>
       ...
       and RFLX.Messages.Msg.Well_Formed (Ctx, Fld)
       and RFLX.Messages.Msg.Valid_Predecessor (Ctx, Fld);
   ...
   function Sufficient_Buffer_Length (Ctx : Context; Fld : Field) return Boolean is
    ...
    with
     Pre =>
       RFLX.Messages.Msg.Has_Buffer (Ctx)
       and RFLX.Messages.Msg.Valid_Next (Ctx, Fld);
   ...
   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer with
     Pre =>
       ...
       and then RFLX.Messages.Msg.Sufficient_Buffer_Length (Ctx, Fld)
   is
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       RFLX.Messages.Msg.Has_Buffer (Ctx)
       and then RFLX.Messages.Msg.Valid_Next (Ctx, Fld)
       and then RFLX.Messages.Msg.Valid_Value (Fld, Val)
       and then RFLX.Messages.Msg.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Messages.Msg.Available_Space (Ctx, Fld)
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.Messages.Msg.Valid_Value (Fld, Val)
       and then RFLX.Messages.Msg.Valid_Size (Ctx, Fld, RFLX.Messages.Msg.Field_Size (Ctx, Fld))
       and then RFLX.Messages.Msg.Available_Space (Ctx, Fld) >= RFLX.Messages.Msg.Field_Size (Ctx, Fld)
       and then RFLX.Messages.Msg.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Messages.Msg.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
       and Invalid_Successor (Ctx, Fld)
   ...
   begin
      Set_Scalar (Ctx, F_A, RFLX.Messages.To_Base_Integer (Val));
   end Set_A;

   procedure Set_B (Ctx : in out Context; Val : RFLX.Messages.Enum_T) is
   begin
      Set_Scalar (Ctx, F_B, RFLX.Messages.To_Base_Integer (Val));
   end Set_B;
