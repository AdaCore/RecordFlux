...
is
   ...
   function Get_Extension (Ctx : Context) return RFLX_Types.Bytes with
     ...
     Pre =>
       RFLX.Test.Message.Has_Buffer (Ctx)
       and then RFLX.Test.Message.Well_Formed (Ctx, RFLX.Test.Message.F_Extension)
   ...
   procedure Get_Extension (Ctx : Context; Data : out RFLX_Types.Bytes) with
     Pre =>
       ...
       and then RFLX.Test.Message.Well_Formed (Ctx, RFLX.Test.Message.F_Extension)
       and then RFLX.Test.Message.Valid_Next (Ctx, RFLX.Test.Message.F_Extension)
       and then Data'Length = RFLX_Types.To_Length (RFLX.Test.Message.Field_Size (Ctx, RFLX.Test.Message.F_Extension)),
     Post =>
       Equal (Ctx, F_Extension, Data);
   ...
   procedure Generic_Get_Extension (Ctx : Context) with
     Pre =>
       RFLX.Test.Message.Has_Buffer (Ctx)
       and RFLX.Test.Message.Present (Ctx, RFLX.Test.Message.F_Extension);
   ...
   procedure Initialize_Extension (Ctx : in out Context) with
     Pre =>
       ...
       and then RFLX.Test.Message.Has_Buffer (Ctx)
       and then RFLX.Test.Message.Valid_Next (Ctx, RFLX.Test.Message.F_Extension)
       and then RFLX.Test.Message.Available_Space (Ctx, RFLX.Test.Message.F_Extension) >= RFLX.Test.Message.Field_Size (Ctx, RFLX.Test.Message.F_Extension),
     Post =>
       Has_Buffer (Ctx)
       and then Well_Formed (Ctx, F_Extension)
       and then (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Extension))
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
       and then Ctx.Length = Ctx.Length'Old
       and then Ctx.Extended = Ctx.Extended'Old
       and then Predecessor (Ctx, F_Extension) = Predecessor (Ctx, F_Extension)'Old
   ...
   procedure Set_Extension (Ctx : in out Context; Data : RFLX_Types.Bytes) with
     Pre =>
       ...
       and then RFLX.Test.Message.Valid_Next (Ctx, RFLX.Test.Message.F_Extension)
       and then RFLX.Test.Message.Available_Space (Ctx, RFLX.Test.Message.F_Extension) >= RFLX.Test.Message.Field_Size (Ctx, RFLX.Test.Message.F_Extension)
       and then RFLX.Test.Message.Valid_Length (Ctx, RFLX.Test.Message.F_Extension, Data'Length)
       and then RFLX.Test.Message.Available_Space (Ctx, RFLX.Test.Message.F_Extension) >= Data'Length * RFLX_Types.Byte'Size
       and then RFLX.Test.Message.Field_Condition (Ctx, RFLX.Test.Message.F_Extension),
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Extension)
       and (if Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Extension))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Length : Test.Length; Extended : Boolean) return Boolean is
     ((if Well_Formed (Cursors (F_Data)) then Cursors (F_Data).Predecessor = F_Initial)
      and then (if
   ...
   function Valid_Next_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Length : Test.Length; Extended : Boolean; Fld : Field) return Boolean is
     ((case Fld is
          when F_Data =>
             Cursors (F_Data).Predecessor = F_Initial,
          when F_Extension =>
             (Well_Formed (Cursors (F_Data))
              and then RFLX_Types.Base_Integer (To_Base_Integer (Extended)) = RFLX_Types.Base_Integer (To_Base_Integer (True))
              and then Cursors (F_Extension).Predecessor = F_Data)))
   ...
   function Field_First (Ctx : Context; Fld : Field) return RFLX_Types.Bit_Index is
     (Field_First_Internal (Ctx.Cursors, Ctx.First, Ctx.Verified_Last, Ctx.Written_Last, Ctx.Buffer, Ctx.Length, Ctx.Extended, Fld));
   ...
   function Valid_Next (Ctx : Context; Fld : Field) return Boolean is
     (Valid_Next_Internal (Ctx.Cursors, Ctx.First, Ctx.Verified_Last, Ctx.Written_Last, Ctx.Buffer, Ctx.Length, Ctx.Extended, Fld));
