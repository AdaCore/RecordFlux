...
is
   ...
   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer with
     Pre =>
       ...
       and then RFLX.TLV.Message.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.TLV.Message.Composite_Field (Fld)
   is
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       ...
       and then RFLX.TLV.Message.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.TLV.Message.Available_Space (Ctx, Fld)
       and then (if RFLX.TLV.Message.Composite_Field (Fld) then Size mod RFLX_Types.Byte'Size = 0 else State_Valid),
     Post =>
       Valid_Next (Ctx, Fld)
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.TLV.Message.Valid_Value (Fld, Val)
       and then RFLX.TLV.Message.Valid_Size (Ctx, Fld, RFLX.TLV.Message.Field_Size (Ctx, Fld))
       and then RFLX.TLV.Message.Available_Space (Ctx, Fld) >= RFLX.TLV.Message.Field_Size (Ctx, Fld)
       and then RFLX.TLV.Message.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.TLV.Message.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
       and Invalid_Successor (Ctx, Fld)
   ...
   procedure Initialize_Value_Private (Ctx : in out Context; Length : RFLX_Types.Length) with
     Pre =>
       ...
       and then RFLX.TLV.Message.Field_First (Ctx, RFLX.TLV.Message.F_Value) mod RFLX_Types.Byte'Size = 1,
     Post =>
       Has_Buffer (Ctx)
   ...
   begin
      ...
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Value) := (State => S_Well_Formed, First => First, Last => Last, Value => 0, Predecessor => Ctx.Cursors (F_Value).Predecessor);
      Ctx.Cursors (Successor (Ctx, F_Value)) := (State => S_Invalid, Predecessor => F_Value, others => <>);
