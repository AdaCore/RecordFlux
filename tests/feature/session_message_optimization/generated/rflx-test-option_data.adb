...
is
   ...
   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer with
     Pre =>
       ...
       and then RFLX.Test.Option_Data.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.Test.Option_Data.Composite_Field (Fld)
   is
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       ...
       and then RFLX.Test.Option_Data.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Test.Option_Data.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.Test.Option_Data.Composite_Field (Fld)
                 then
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.Test.Option_Data.Valid_Value (Fld, Val)
       and then RFLX.Test.Option_Data.Valid_Size (Ctx, Fld, RFLX.Test.Option_Data.Field_Size (Ctx, Fld))
       and then RFLX.Test.Option_Data.Available_Space (Ctx, Fld) >= RFLX.Test.Option_Data.Field_Size (Ctx, Fld)
       and then RFLX.Test.Option_Data.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Test.Option_Data.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Initialize_Data_Private (Ctx : in out Context; Length : RFLX_Types.Length) with
     Pre =>
       ...
       and then RFLX.Test.Option_Data.Field_First (Ctx, RFLX.Test.Option_Data.F_Data) mod RFLX_Types.Byte'Size = 1,
     Post =>
       Has_Buffer (Ctx)
