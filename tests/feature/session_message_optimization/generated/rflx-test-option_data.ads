...
is
   ...
   procedure Copy (Ctx : Context; Buffer : out RFLX_Types.Bytes) with
     Pre =>
       RFLX.Test.Option_Data.Has_Buffer (Ctx)
       and then RFLX.Test.Option_Data.Well_Formed_Message (Ctx)
   ...
   function Get_Data (Ctx : Context) return RFLX_Types.Bytes with
     ...
     Pre =>
       RFLX.Test.Option_Data.Has_Buffer (Ctx)
       and then RFLX.Test.Option_Data.Well_Formed (Ctx, RFLX.Test.Option_Data.F_Data)
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.Universal.Length) with
     ...
     Post =>
       ...
       and (if Get_Length (Ctx) > 1 then Valid_Next (Ctx, F_Data))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
