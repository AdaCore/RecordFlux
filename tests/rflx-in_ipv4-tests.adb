...
package body RFLX.In_IPv4.Tests is
   ...
   Data : Builtin_Types.Bytes (Builtin_Types.Index'First .. Builtin_Types.Index'First + 25) := (others => 0);
   ...
   is
      ...
      Buffer                 : Builtin_Types.Bytes_Ptr := new Builtin_Types.Bytes'(Builtin_Types.Index'First .. Builtin_Types.Index'First + Expected'Size - 1 => 0);
      Ethernet_Frame_Context : Ethernet.Frame.Context := Ethernet.Frame.Create;
      IPv4_Packet_Context    : IPv4.Packet.Context := IPv4.Packet.Create;
      UDP_Datagram_Context   : UDP.Datagram.Context := UDP.Datagram.Create;
   begin
      ...
      if In_Ethernet.Contains.IPv4_Packet_In_Ethernet_Frame_Payload (Ethernet_Frame_Context) then
         ...
         IPv4.Packet.Set_Flag_R (IPv4_Packet_Context, IPv4.Flag_False);
         IPv4.Packet.Set_Flag_DF (IPv4_Packet_Context, IPv4.Flag_False);
         IPv4.Packet.Set_Flag_MF (IPv4_Packet_Context, IPv4.Flag_False);
         ...
         if In_IPv4.Contains.UDP_Datagram_In_IPv4_Packet_Payload (IPv4_Packet_Context) then
            ...
            Assert (Builtin_Types.Length'Image (Types.Byte_Index (UDP_Datagram_Context.Last) - Types.Byte_Index (Ethernet_Frame_Context.First) + 1), Expected'Length'Img, "Invalid buffer length");
            Assert (Buffer.all (Types.Byte_Index (Ethernet_Frame_Context.First) .. Types.Byte_Index (Ethernet_Frame_Context.Last)), Expected.all, "Invalid binary representation");
         end if;
      end if;
