...
is
   ...
   begin
      ...
      then
         P_Next_State := S_Prepare_Message;
      else
         P_Next_State := S_Terminated;
      end if;
   end Start;

   procedure Prepare_Message (P_Next_State : out State) with
     Pre =>
       Initialized,
     Post =>
       Initialized
   ...
   procedure Next (P_Next_State : out State) with
     Pre =>
       Initialized,
     Post =>
       Initialized
   ...
   begin
      M_Buffer := Test.Session_Allocator.Slot_Ptr_2;
      ...
      if RFLX_Types.To_First_Bit_Index (M_Ctx.Buffer_Last) - RFLX_Types.To_First_Bit_Index (M_Ctx.Buffer_First) + 1 >= RFLX_Types.Bit_Length (32) then
         ...
         if Universal.Message.Field_Size (M_Ctx, Universal.Message.F_Data) = 1 * RFLX_Types.Byte'Size then
            ...
            pragma Warnings (Off, "unused assignment");
            Test.Session_Allocator.Slot_Ptr_2 := M_Buffer;
            pragma Warnings (On, "unused assignment");
            return;
         end if;
      else
   ...
   begin
      case P_Next_State is
         ...
         when S_Prepare_Message | S_Reply | S_Next | S_Terminated =>
            null;
      end case;
