...
is
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.Test.Length) with
     ...
     Pre =>
       ...
       and then RFLX.Test.Valid_Length (To_S63 (Val))
   ...
   procedure Set_Data_Empty (Ctx : in out Context) with
     Pre =>
       ...
       and then Has_Buffer (Ctx)
       and then Valid_Next (Ctx, F_Data)
       and then Field_Condition (Ctx, F_Data)
...
private
   ...
   function Sufficient_Buffer_Length (Ctx : Context; Struct : Structure) return Boolean is
     (RFLX_Types.S63 (RFLX_Types.To_Last_Bit_Index (Ctx.Buffer_Last) - RFLX_Types.To_First_Bit_Index (Ctx.Buffer_First) + 1) >= RFLX_Types.S63 (Struct.Length) * 8 + 16);
