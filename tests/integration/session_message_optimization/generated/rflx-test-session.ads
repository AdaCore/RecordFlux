...
is
   ...
   type State is (S_Start, S_Process, S_Reply, S_Trigger_Error, S_Error, S_Final);
   ...
   procedure Get_Option_Data (Ctx : in out Context; Data : RFLX_Types.Bytes; RFLX_Result : out RFLX.Test.Option_Data.Structure) is abstract;
...
private
   ...
   type Private_Context is
      record
         ...
         Option_Ctx : Universal.Option.Context;
   ...
   function Uninitialized (Ctx : Context'Class) return Boolean is
     (not Universal.Message.Has_Buffer (Ctx.P.Message_Ctx)
      and not Universal.Option.Has_Buffer (Ctx.P.Option_Ctx)
   ...
   function Global_Initialized (Ctx : Context'Class) return Boolean is
     (Universal.Message.Has_Buffer (Ctx.P.Message_Ctx)
      ...
      and then Universal.Option.Has_Buffer (Ctx.P.Option_Ctx)
      and then Ctx.P.Option_Ctx.Buffer_First = RFLX_Types.Index'First
      and then Ctx.P.Option_Ctx.Buffer_Last = RFLX_Types.Index'First + 4095);
     ...
     ((case Chan is
          when C_Channel =>
             (case Ctx.P.Next_State is
                 when S_Reply =>
                    Universal.Option.Structural_Valid_Message (Ctx.P.Option_Ctx)
                    and Universal.Option.Byte_Size (Ctx.P.Option_Ctx) > 0,
                 when others =>
                    False)));
   ...
   function Read_Buffer_Size (Ctx : Context'Class; Chan : Channel) return RFLX_Types.Length is
     ((case Chan is
          when C_Channel =>
             (case Ctx.P.Next_State is
                 when S_Reply =>
                    Universal.Option.Byte_Size (Ctx.P.Option_Ctx),
                 when others =>
