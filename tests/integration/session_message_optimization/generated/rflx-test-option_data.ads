...
package RFLX.Test.Option_Data with
  SPARK_Mode,
  Annotate =>
...
is
   ...
   type Virtual_Field is (F_Initial, F_Length, F_Data, F_Final);

   subtype Field is Virtual_Field range F_Length .. F_Data;
   ...
   procedure Reset (Ctx : in out Context) with
     Pre =>
       not Ctx'Constrained
       and RFLX.Test.Option_Data.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Copy (Ctx : Context; Buffer : out RFLX_Types.Bytes) with
     Pre =>
       RFLX.Test.Option_Data.Has_Buffer (Ctx)
       and then RFLX.Test.Option_Data.Structural_Valid_Message (Ctx)
       and then RFLX.Test.Option_Data.Byte_Size (Ctx) = Buffer'Length;
   ...
   procedure Generic_Write (Ctx : in out Context; Offset : RFLX_Types.Length := 0) with
     Pre =>
       ...
       and then Offset < RFLX.Test.Option_Data.Buffer_Length (Ctx)
       and then Pre (RFLX.Test.Option_Data.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   procedure Data (Ctx : Context; Data : out RFLX_Types.Bytes) with
     Pre =>
       RFLX.Test.Option_Data.Has_Buffer (Ctx)
       and then RFLX.Test.Option_Data.Structural_Valid_Message (Ctx)
       and then Data'Length = RFLX.Test.Option_Data.Byte_Size (Ctx);
   ...
   function Path_Condition (Ctx : Context; Fld : Field) return Boolean with
     Pre =>
       RFLX.Test.Option_Data.Valid_Predecessor (Ctx, Fld),
     Post =>
       True;
   ...
   function Get_Length (Ctx : Context) return RFLX.Universal.Length with
     Pre =>
       RFLX.Test.Option_Data.Valid (Ctx, RFLX.Test.Option_Data.F_Length);
   ...
   function Get_Data (Ctx : Context) return RFLX_Types.Bytes with
     ...
     Pre =>
       RFLX.Test.Option_Data.Has_Buffer (Ctx)
       and then RFLX.Test.Option_Data.Structural_Valid (Ctx, RFLX.Test.Option_Data.F_Data)
   ...
   procedure Get_Data (Ctx : Context; Data : out RFLX_Types.Bytes) with
     Pre =>
       ...
       and then RFLX.Test.Option_Data.Structural_Valid (Ctx, RFLX.Test.Option_Data.F_Data)
       and then RFLX.Test.Option_Data.Valid_Next (Ctx, RFLX.Test.Option_Data.F_Data)
       and then Data'Length = RFLX_Types.To_Length (RFLX.Test.Option_Data.Field_Size (Ctx, RFLX.Test.Option_Data.F_Data)),
     Post =>
   ...
   procedure Generic_Get_Data (Ctx : Context) with
     Pre =>
       RFLX.Test.Option_Data.Has_Buffer (Ctx)
       and RFLX.Test.Option_Data.Present (Ctx, RFLX.Test.Option_Data.F_Data);
   ...
   procedure Generic_Set_Data (Ctx : in out Context; Length : RFLX_Types.Length) with
     Pre =>
       ...
       and then RFLX.Test.Option_Data.Valid_Next (Ctx, RFLX.Test.Option_Data.F_Data)
       and then RFLX.Test.Option_Data.Available_Space (Ctx, RFLX.Test.Option_Data.F_Data) >= RFLX.Test.Option_Data.Field_Size (Ctx, RFLX.Test.Option_Data.F_Data)
       and then RFLX.Test.Option_Data.Valid_Length (Ctx, RFLX.Test.Option_Data.F_Data, Length)
       and then RFLX_Types.To_Length (RFLX.Test.Option_Data.Available_Space (Ctx, RFLX.Test.Option_Data.F_Data)) >= Length
   ...
   procedure To_Context (Struct : Structure; Ctx : in out Context) with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Test.Option_Data.Has_Buffer (Ctx)
       and then RFLX.Test.Option_Data.Valid_Structure (Struct)
       and then RFLX.Test.Option_Data.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
...
private
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      ...
      and then RFLX.Test.Option_Data.Field_First (Ctx, RFLX.Test.Option_Data.F_Length) rem RFLX_Types.Byte'Size = 1
