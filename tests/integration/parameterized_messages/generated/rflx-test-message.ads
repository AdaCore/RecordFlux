...
is
   ...
   procedure Set_Extension (Ctx : in out Context; Data : RFLX_Types.Bytes) with
     Pre =>
       ...
       and then Available_Space (Ctx, F_Extension) >= Data'Length * RFLX_Types.Byte'Size
       and then Field_Condition (Ctx, F_Extension),
     Post =>
       Has_Buffer (Ctx)
       and Structural_Valid (Ctx, F_Extension)
       and (if Structural_Valid_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Extension))
...
private
      ...
      and then ((if
                 ...
                 then
                    (Structural_Valid (Cursors (F_Data))
                     ...
                     and then RFLX_Types.U64 (To_U64 (Extended)) = RFLX_Types.U64 (To_U64 (True)))))
   ...
   function Field_Condition (Ctx : Context; Fld : Field) return Boolean is
     ((case Fld is
          when F_Data =>
             RFLX_Types.U64 (To_U64 (Ctx.Extended)) = RFLX_Types.U64 (To_U64 (True))
             or RFLX_Types.U64 (To_U64 (Ctx.Extended)) = RFLX_Types.U64 (To_U64 (False)),
          when F_Extension =>
             True));
   ...
   function Structural_Valid_Message (Ctx : Context) return Boolean is
     ((Structural_Valid (Ctx, F_Data)
       and then RFLX_Types.U64 (To_U64 (Ctx.Extended)) = RFLX_Types.U64 (To_U64 (False)))
