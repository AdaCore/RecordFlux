...
is
   ...
   procedure Set_Field_Value_C (Ctx : in out Context; Val : Field_Dependent_Value; Fst, Lst : out RFLX_Types.Bit_Index) with
     Pre =>
       not Ctx'Constrained
       and then Has_Buffer (Ctx)
       and then F_C in Field'Range
       and then Valid_Next (Ctx, F_C)
       and then Available_Space (Ctx, F_C) >= Field_Size (Ctx, F_C)
       and then (for all F in Field'Range =>
                    (if Structural_Valid (Ctx.Cursors (F)) then Ctx.Cursors (F).Last <= Field_Last (Ctx, F_C))),
     Post =>
       Has_Buffer (Ctx)
       and Fst = Field_First (Ctx, F_C)
       and Lst = Field_Last (Ctx, F_C)
       and Fst >= Ctx.First
       and Fst <= Lst + 1
       and Lst <= Ctx.Last
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_C);
      Last : constant RFLX_Types.Bit_Index := Field_Last (Ctx, F_C);
      function Buffer_First return RFLX_Types.Index is
        (RFLX_Types.To_Index (First));
      function Buffer_Last return RFLX_Types.Index is
        (RFLX_Types.To_Index (Last));
   ...
   end Set_Field_Value_C;

   procedure Set_Field_Value_D (Ctx : in out Context; Val : Field_Dependent_Value; Fst, Lst : out RFLX_Types.Bit_Index) with
     Pre =>
       not Ctx'Constrained
       and then Has_Buffer (Ctx)
       and then F_D in Field'Range
       and then Valid_Next (Ctx, F_D)
       and then Available_Space (Ctx, F_D) >= Field_Size (Ctx, F_D)
       and then (for all F in Field'Range =>
                    (if Structural_Valid (Ctx.Cursors (F)) then Ctx.Cursors (F).Last <= Field_Last (Ctx, F_D))),
     Post =>
       Has_Buffer (Ctx)
       and Fst = Field_First (Ctx, F_D)
       and Lst = Field_Last (Ctx, F_D)
       and Fst >= Ctx.First
       and Fst <= Lst + 1
       and Lst <= Ctx.Last
       and (for all F in Field'Range =>
       ...
       and Ctx.Cursors = Ctx.Cursors'Old
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_D);
      Last : constant RFLX_Types.Bit_Index := Field_Last (Ctx, F_D);
      function Buffer_First return RFLX_Types.Index is
        (RFLX_Types.To_Index (First));
      function Buffer_Last return RFLX_Types.Index is
        (RFLX_Types.To_Index (Last));
   ...
   begin
      Reset_Dependent_Fields (Ctx, F_C);
      Set_Field_Value_C (Ctx, Field_Value, First, Last);
      ...
      Ctx.Cursors (F_C) := (State => S_Valid, First => First, Last => Last, Value => Field_Value, Predecessor => Ctx.Cursors (F_C).Predecessor);
   ...
   procedure Set_D (Ctx : in out Context; Val : RFLX.Messages.Enum_T) is
      ...
      First, Last : RFLX_Types.Bit_Index;
   begin
      Reset_Dependent_Fields (Ctx, F_D);
      Set_Field_Value_D (Ctx, Field_Value, First, Last);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_D) := (State => S_Valid, First => First, Last => Last, Value => Field_Value, Predecessor => Ctx.Cursors (F_D).Predecessor);
      Ctx.Cursors (Successor (Ctx, F_D)) := (State => S_Invalid, Predecessor => F_D);
   end Set_D;
