   ...
   function Has_Data (Chan : Channel) return Boolean is
     ((case Chan is
          when C_Channel =>
             (case P_Next_State is
                 when S_Reply =>
                    Universal.Option.Structural_Valid_Message (Inner_Message_Ctx)
                    and Universal.Option.Byte_Size (Inner_Message_Ctx) > 0,
                 when others =>
                    False)));
   ...
   function Read_Buffer_Size (Chan : Channel) return RFLX_Types.Length is
     ((case Chan is
          when C_Channel =>
             (case P_Next_State is
                 when S_Reply =>
                    Universal.Option.Byte_Size (Inner_Message_Ctx),
                 when others =>
                    raise Program_Error)));
