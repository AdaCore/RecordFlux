...
private
   ...
   function Uninitialized (Ctx : Context'Class) return Boolean is
     (not Universal.Message.Has_Buffer (Ctx.P.Message_Ctx)
      and not Universal.Option.Has_Buffer (Ctx.P.Inner_Message_Ctx)
   ...
   function Global_Initialized (Ctx : Context'Class) return Boolean is
     (Universal.Message.Has_Buffer (Ctx.P.Message_Ctx)
      ...
      and then Universal.Option.Has_Buffer (Ctx.P.Inner_Message_Ctx)
      and then Ctx.P.Inner_Message_Ctx.Buffer_First = RFLX_Types.Index'First
      and then Ctx.P.Inner_Message_Ctx.Buffer_Last = RFLX_Types.Index'First + 4095);
     ...
     ((case Chan is
          when C_Channel =>
             (case Ctx.P.Next_State is
                 when S_Reply =>
                    Universal.Option.Structural_Valid_Message (Ctx.P.Inner_Message_Ctx)
                    and Universal.Option.Byte_Size (Ctx.P.Inner_Message_Ctx) > 0,
                 when others =>
                    False)));
   ...
   function Read_Buffer_Size (Ctx : Context'Class; Chan : Channel) return RFLX_Types.Length is
     ((case Chan is
          when C_Channel =>
             (case Ctx.P.Next_State is
                 when S_Reply =>
                    Universal.Option.Byte_Size (Ctx.P.Inner_Message_Ctx),
                 when others =>
