...
is
   ...
   procedure Set_Tag (Ctx : in out Context; Val : RFLX.TLV.Tag) with
     ...
     Post =>
       ...
       and (if
               RFLX_Types.U64 (To_Base (Get_Tag (Ctx))) = RFLX_Types.U64 (To_Base (Msg_Data))
            then
...
private
      ...
      and then ((if
                 ...
                 then
                    (Valid (Cursors (F_Tag))
                     ...
                     and then RFLX_Types.U64 (Cursors (F_Tag).Value.Tag_Value) = RFLX_Types.U64 (To_Base (Msg_Data))))
                and then (if
   ...
   function Field_Condition (Ctx : Context; Val : Field_Dependent_Value) return Boolean is
     ((case Val.Fld is
          when F_Initial =>
             True,
          when F_Tag =>
             RFLX_Types.U64 (Val.Tag_Value) = RFLX_Types.U64 (To_Base (Msg_Error))
             or RFLX_Types.U64 (Val.Tag_Value) = RFLX_Types.U64 (To_Base (Msg_Data)),
     ...
     ((case Fld is
          ...
          when F_Length =>
             (if
                 ...
                 and then RFLX_Types.U64 (Ctx.Cursors (F_Tag).Value.Tag_Value) = RFLX_Types.U64 (To_Base (Msg_Data))
              then
   ...
   function Structural_Valid_Message (Ctx : Context) return Boolean is
     (Valid (Ctx, F_Tag)
      and then (RFLX_Types.U64 (Ctx.Cursors (F_Tag).Value.Tag_Value) = RFLX_Types.U64 (To_Base (Msg_Error))
