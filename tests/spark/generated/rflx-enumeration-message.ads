...
is
   ...
   procedure Set_Priority (Ctx : in out Context; Val : RFLX.Enumeration.Priority_Enum) with
     ...
     Pre =>
       ...
       and then RFLX.Enumeration.Valid_Priority (To_S63 (Val))
...
private
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Verified_Last = Ctx.First - 1
      and then Valid_Next (Ctx, F_Priority)
      and then Field_First (Ctx, F_Priority) rem RFLX_Types.Byte'Size = 1
   ...
   function Sufficient_Buffer_Length (Ctx : Context; Unused_Struct : Structure) return Boolean is
     (RFLX_Types.S63 (RFLX_Types.To_Last_Bit_Index (Ctx.Buffer_Last) - RFLX_Types.To_First_Bit_Index (Ctx.Buffer_First) + 1) >= 8);
