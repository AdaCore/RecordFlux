...
is
   ...
   procedure Reset (Ctx : in out Context) with
     Pre =>
       not Ctx'Constrained
       and RFLX.Enumeration.Message.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Copy (Ctx : Context; Buffer : out RFLX_Types.Bytes) with
     Pre =>
       RFLX.Enumeration.Message.Has_Buffer (Ctx)
       and then RFLX.Enumeration.Message.Well_Formed_Message (Ctx)
       and then RFLX.Enumeration.Message.Byte_Size (Ctx) = Buffer'Length;
   ...
   procedure Generic_Write (Ctx : in out Context; Offset : RFLX_Types.Length := 0) with
     Pre =>
       ...
       and then Offset < RFLX.Enumeration.Message.Buffer_Length (Ctx)
       and then Pre (RFLX.Enumeration.Message.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   procedure Data (Ctx : Context; Data : out RFLX_Types.Bytes) with
     Pre =>
       RFLX.Enumeration.Message.Has_Buffer (Ctx)
       and then RFLX.Enumeration.Message.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.Enumeration.Message.Byte_Size (Ctx);
   ...
   function Field_Condition (Ctx : Context; Fld : Field) return Boolean with
     Pre =>
       ...
       and then RFLX.Enumeration.Message.Sufficient_Space (Ctx, Fld),
     Post =>
       True;
   ...
   function Get_Priority (Ctx : Context) return RFLX.Enumeration.Priority with
     Pre =>
       RFLX.Enumeration.Message.Valid (Ctx, RFLX.Enumeration.Message.F_Priority);
   ...
   procedure Set_Priority (Ctx : in out Context; Val : RFLX.Enumeration.Priority_Enum) with
     ...
     Pre =>
       ...
       and then RFLX.Enumeration.Valid_Priority (RFLX.Enumeration.To_Base_Integer (Val))
   ...
   procedure To_Context (Struct : Structure; Ctx : in out Context) with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Enumeration.Message.Has_Buffer (Ctx)
       and then RFLX.Enumeration.Message.Valid_Structure (Struct)
       and then RFLX.Enumeration.Message.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
   ...
   function Field_Size_Priority (Struct : Structure) return RFLX_Types.Bit_Length with
     Pre =>
       Valid_Structure (Struct);
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr) return Boolean is
     ((if Well_Formed (Cursors (F_Priority)) then Cursors (F_Priority).Predecessor = F_Initial))
    with
     Pre =>
   ...
   function Valid_Next_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Fld : Field) return Boolean is
     ((case Fld is
          when F_Priority =>
             Cursors (F_Priority).Predecessor = F_Initial))
    with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
   ...
   function Get_Priority (Ctx : Context) return RFLX.Enumeration.Priority is
     (To_Actual (Ctx.Cursors (F_Priority).Value));
   ...
   function Field_Size_Priority (Struct : Structure) return RFLX_Types.Bit_Length is
     (8);
