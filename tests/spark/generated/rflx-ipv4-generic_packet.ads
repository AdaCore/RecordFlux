...
is
   ...
   procedure Set_Options_Empty (Ctx : in out Context) with
     Pre =>
       ...
       and then Field_Last (Ctx, F_Options) mod Types.Byte'Size = 0
...
private
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Message_Last = Ctx.First - 1
      and then Valid_Next (Ctx, F_Version)
      and then Field_First (Ctx, F_Version) mod Types.Byte'Size = 1
   ...
   function Field_Condition (Ctx : Context; Val : Field_Dependent_Value) return Boolean is
     ((case Val.Fld is
          when F_Initial | F_Version | F_IHL | F_DSCP | F_ECN =>
             True,
          when F_Total_Length =>
             Types.U64 (Val.Total_Length_Value) >= Types.U64 (Ctx.Cursors (F_IHL).Value.IHL_Value) * 4,
          when F_Identification =>
             True,
          when F_Flag_R =>
             Types.U64 (Val.Flag_R_Value) = Types.U64 (To_Base (False)),
