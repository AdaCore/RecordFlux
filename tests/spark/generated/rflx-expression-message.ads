...
is
   ...
   procedure Reset (Ctx : in out Context) with
     Pre =>
       not Ctx'Constrained
       and RFLX.Expression.Message.Has_Buffer (Ctx),
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Copy (Ctx : Context; Buffer : out RFLX_Types.Bytes) with
     Pre =>
       RFLX.Expression.Message.Has_Buffer (Ctx)
       and then RFLX.Expression.Message.Well_Formed_Message (Ctx)
       and then RFLX.Expression.Message.Byte_Size (Ctx) = Buffer'Length;
   ...
   procedure Generic_Write (Ctx : in out Context; Offset : RFLX_Types.Length := 0) with
     Pre =>
       ...
       and then Offset < RFLX.Expression.Message.Buffer_Length (Ctx)
       and then Pre (RFLX.Expression.Message.Buffer_Length (Ctx), Offset),
     Post =>
       Has_Buffer (Ctx)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
   ...
   procedure Data (Ctx : Context; Data : out RFLX_Types.Bytes) with
     Pre =>
       RFLX.Expression.Message.Has_Buffer (Ctx)
       and then RFLX.Expression.Message.Well_Formed_Message (Ctx)
       and then Data'Length = RFLX.Expression.Message.Byte_Size (Ctx);
   ...
   function Get_Payload (Ctx : Context) return RFLX_Types.Bytes with
     ...
     Pre =>
       RFLX.Expression.Message.Has_Buffer (Ctx)
       and then RFLX.Expression.Message.Well_Formed (Ctx, RFLX.Expression.Message.F_Payload)
   ...
   procedure Get_Payload (Ctx : Context; Data : out RFLX_Types.Bytes) with
     Pre =>
       ...
       and then RFLX.Expression.Message.Well_Formed (Ctx, RFLX.Expression.Message.F_Payload)
       and then RFLX.Expression.Message.Valid_Next (Ctx, RFLX.Expression.Message.F_Payload)
       and then Data'Length = RFLX_Types.To_Length (RFLX.Expression.Message.Field_Size (Ctx, RFLX.Expression.Message.F_Payload)),
     Post =>
   ...
   procedure Generic_Get_Payload (Ctx : Context) with
     Pre =>
       RFLX.Expression.Message.Has_Buffer (Ctx)
       and RFLX.Expression.Message.Present (Ctx, RFLX.Expression.Message.F_Payload);
   ...
   procedure Initialize_Payload (Ctx : in out Context) with
     Pre =>
       ...
       and then RFLX.Expression.Message.Has_Buffer (Ctx)
       and then RFLX.Expression.Message.Valid_Next (Ctx, RFLX.Expression.Message.F_Payload)
       and then RFLX.Expression.Message.Available_Space (Ctx, RFLX.Expression.Message.F_Payload) >= RFLX.Expression.Message.Field_Size (Ctx, RFLX.Expression.Message.F_Payload),
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure Set_Payload (Ctx : in out Context; Data : RFLX_Types.Bytes) with
     Pre =>
       ...
       and then RFLX.Expression.Message.Valid_Next (Ctx, RFLX.Expression.Message.F_Payload)
       and then RFLX.Expression.Message.Available_Space (Ctx, RFLX.Expression.Message.F_Payload) >= RFLX.Expression.Message.Field_Size (Ctx, RFLX.Expression.Message.F_Payload)
       and then RFLX.Expression.Message.Valid_Length (Ctx, RFLX.Expression.Message.F_Payload, Data'Length)
       and then RFLX.Expression.Message.Available_Space (Ctx, RFLX.Expression.Message.F_Payload) >= Data'Length * RFLX_Types.Byte'Size
       and then RFLX.Expression.Message.Field_Condition (Ctx, RFLX.Expression.Message.F_Payload, Data),
     Post =>
       Has_Buffer (Ctx)
   ...
   procedure To_Context (Struct : Structure; Ctx : in out Context) with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Expression.Message.Has_Buffer (Ctx)
       and then RFLX.Expression.Message.Valid_Structure (Struct)
       and then RFLX.Expression.Message.Sufficient_Buffer_Length (Ctx, Struct),
     Post =>
       Has_Buffer (Ctx)
...
private
   ...
   function Valid_Predecessors_Invariant (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr) return Boolean is
     ((if Well_Formed (Cursors (F_Payload)) then Cursors (F_Payload).Predecessor = F_Initial))
    with
     Pre =>
   ...
   function Valid_Next_Internal (Cursors : Field_Cursors; First : RFLX_Types.Bit_Index; Verified_Last : RFLX_Types.Bit_Length; Written_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Fld : Field) return Boolean is
     ((case Fld is
          when F_Payload =>
             Cursors (F_Payload).Predecessor = F_Initial))
    with
     Pre =>
       Cursors_Invariant (Cursors, First, Verified_Last)
