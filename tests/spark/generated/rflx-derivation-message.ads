...
package RFLX.Derivation.Message
with
  SPARK_Mode,
  Always_Terminates
is
   ...
   function Get_Tag (Ctx : Context) return RFLX.TLV.Tag
   with
     Pre =>
       RFLX.Derivation.Message.Valid (Ctx, RFLX.Derivation.Message.F_Tag);

   function Get_Length (Ctx : Context) return RFLX.TLV.Length
   with
     Pre =>
   ...
   procedure Set_Tag (Ctx : in out Context; Val : RFLX.TLV.Tag)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.TLV.Length)
   with
     Inline_Always,
     Pre =>
       not Ctx'Constrained
   ...
   procedure Set_Value (Ctx : in out Context; Data : RFLX_Types.Bytes)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Derivation.Message.Has_Buffer (Ctx)
   ...
   procedure Generic_Set_Value (Ctx : in out Context; Length : RFLX_Types.Length)
   with
     Pre =>
       not Ctx'Constrained
       and then RFLX.Derivation.Message.Has_Buffer (Ctx)
