...
is
   ...
   begin
      Ctx := (Buffer_First, Buffer_Last, First, Last, First - 1, (if Written_Last = 0 then First - 1 else Written_Last), Buffer, (F_Destination => (State => S_Invalid, Predecessor => F_Initial, others => <>), others => (State => S_Invalid, Predecessor => F_Final, others => <>)));
      Buffer := null;
   end Initialize;
   ...
   begin
      Ctx := (Ctx.Buffer_First, Ctx.Buffer_Last, First, Last, First - 1, First - 1, Ctx.Buffer, (F_Destination => (State => S_Invalid, Predecessor => F_Initial, others => <>), others => (State => S_Invalid, Predecessor => F_Final, others => <>)));
   end Reset;
   ...
   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer with
     Pre =>
       ...
       and then RFLX.Ethernet.Frame.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.Ethernet.Frame.Composite_Field (Fld)
   is
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            if
               ...
               and then Field_Condition (Ctx, Fld, Value, Field_Size (Ctx, Fld))
            then
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       ...
       and then RFLX.Ethernet.Frame.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Ethernet.Frame.Available_Space (Ctx, Fld)
       and then (if RFLX.Ethernet.Frame.Composite_Field (Fld) then Size mod RFLX_Types.Byte'Size = 0 else State_Valid),
     Post =>
       Valid_Next (Ctx, Fld)
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.Ethernet.Frame.Valid_Next (Ctx, Fld)
       and then Fld in F_Destination | F_Source | F_Type_Length_TPID | F_TPID | F_TCI | F_Type_Length
       and then RFLX.Ethernet.Frame.Valid_Value (Fld, Val)
       and then RFLX.Ethernet.Frame.Valid_Size (Ctx, Fld, RFLX.Ethernet.Frame.Field_Size (Ctx, Fld))
       and then RFLX.Ethernet.Frame.Available_Space (Ctx, Fld) >= RFLX.Ethernet.Frame.Field_Size (Ctx, Fld)
       and then RFLX.Ethernet.Frame.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Ethernet.Frame.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
       and Invalid_Successor (Ctx, Fld)
   ...
   procedure Initialize_Payload_Private (Ctx : in out Context; Length : RFLX_Types.Length) with
     Pre =>
       ...
       and then RFLX.Ethernet.Frame.Field_First (Ctx, RFLX.Ethernet.Frame.F_Payload) mod RFLX_Types.Byte'Size = 1,
     Post =>
       Has_Buffer (Ctx)
