...
is
   ...
   procedure Initialize_Messages_Private (Ctx : in out Context; Length : RFLX_Types.Length) with
     ...
     Post =>
       ...
       and then Field_Size (Ctx, F_Messages) = RFLX_Types.To_Bit_Length (Length)
       and then Ctx.Verified_Last = Field_Last (Ctx, F_Messages)
       and then Ctx.Buffer_First = Ctx.Buffer_First'Old
       and then Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and then Ctx.First = Ctx.First'Old
       and then Ctx.Last = Ctx.Last'Old
   ...
   begin
      ...
      if Valid_Sequence then
         Ctx.Cursors (F_Messages) := (State => S_Valid, First => Ctx.Cursors (F_Messages).First, Last => Ctx.Cursors (F_Messages).Last, Value => Ctx.Cursors (F_Messages).Value);
      else
         Reset_Dependent_Fields (Ctx, F_Messages);
         Ctx.Cursors (F_Messages) := (State => S_Invalid, others => <>);
      end if;
   end Update_Messages;
