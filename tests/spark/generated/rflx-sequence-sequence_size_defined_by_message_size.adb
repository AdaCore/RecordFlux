...
is
   ...
   begin
      Ctx := (Buffer_First, Buffer_Last, First, Last, First - 1, (if Written_Last = 0 then First - 1 else Written_Last), Buffer, (F_Header => (State => S_Invalid, Predecessor => F_Initial, others => <>), others => (State => S_Invalid, Predecessor => F_Final, others => <>)));
      Buffer := null;
   end Initialize;
   ...
   begin
      Ctx := (Ctx.Buffer_First, Ctx.Buffer_Last, First, Last, First - 1, First - 1, Ctx.Buffer, (F_Header => (State => S_Invalid, Predecessor => F_Initial, others => <>), others => (State => S_Invalid, Predecessor => F_Final, others => <>)));
   end Reset;
   ...
   function Successor (Ctx : Context; Fld : Field) return Virtual_Field is
    ...
    with
     Pre =>
       ...
       and RFLX.Sequence.Sequence_Size_Defined_By_Message_Size.Valid_Predecessor (Ctx, Fld);
   ...
   begin
      ...
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Vector) := (State => S_Well_Formed, First => First, Last => Last, Value => 0, Predecessor => Ctx.Cursors (F_Vector).Predecessor);
      Ctx.Cursors (Successor (Ctx, F_Vector)) := (State => S_Invalid, Predecessor => F_Vector, others => <>);
   ...
   begin
      ...
      if Valid_Sequence then
         ...
         Reset_Dependent_Fields (Ctx, F_Vector);
         Ctx.Cursors (F_Vector) := (State => S_Invalid, Predecessor => Ctx.Cursors (F_Vector).Predecessor, others => <>);
      end if;
   end Update_Vector;
