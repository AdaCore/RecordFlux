...
private
     ...
     ((case Fld is
          ...
          when F_Code_Destination_Unreachable =>
             (if
                 ...
                 and then RFLX_Types.U64 (Ctx.Cursors (F_Tag).Value.Tag_Value) = RFLX_Types.U64 (To_Base (RFLX.ICMP.Destination_Unreachable))
              then
          ...
          when F_Code_Redirect =>
             (if
                 Ctx.Cursors (Fld).Predecessor = F_Tag
                 and then RFLX_Types.U64 (Ctx.Cursors (F_Tag).Value.Tag_Value) = RFLX_Types.U64 (To_Base (RFLX.ICMP.Redirect))
              then
                 Ctx.Cursors (Ctx.Cursors (Fld).Predecessor).Last + 1
              else
                 raise Program_Error),
          when F_Code_Time_Exceeded =>
             (if
                 Ctx.Cursors (Fld).Predecessor = F_Tag
                 and then RFLX_Types.U64 (Ctx.Cursors (F_Tag).Value.Tag_Value) = RFLX_Types.U64 (To_Base (RFLX.ICMP.Time_Exceeded))
              then
                 Ctx.Cursors (Ctx.Cursors (Fld).Predecessor).Last + 1
              else
                 raise Program_Error),
          when F_Code_Zero =>
             (if
                 Ctx.Cursors (Fld).Predecessor = F_Tag
                 and then (RFLX_Types.U64 (Ctx.Cursors (F_Tag).Value.Tag_Value) = RFLX_Types.U64 (To_Base (RFLX.ICMP.Information_Reply))
                           or RFLX_Types.U64 (Ctx.Cursors (F_Tag).Value.Tag_Value) = RFLX_Types.U64 (To_Base (RFLX.ICMP.Information_Request))
                           or RFLX_Types.U64 (Ctx.Cursors (F_Tag).Value.Tag_Value) = RFLX_Types.U64 (To_Base (RFLX.ICMP.Timestamp_Reply))
                           or RFLX_Types.U64 (Ctx.Cursors (F_Tag).Value.Tag_Value) = RFLX_Types.U64 (To_Base (RFLX.ICMP.Timestamp_Msg))
                           or RFLX_Types.U64 (Ctx.Cursors (F_Tag).Value.Tag_Value) = RFLX_Types.U64 (To_Base (RFLX.ICMP.Parameter_Problem))
          ...
          when F_Pointer =>
             (if
                 Ctx.Cursors (Fld).Predecessor = F_Checksum
                 and then RFLX_Types.U64 (Ctx.Cursors (F_Tag).Value.Tag_Value) = RFLX_Types.U64 (To_Base (RFLX.ICMP.Parameter_Problem))
              then
                 Ctx.Cursors (Ctx.Cursors (Fld).Predecessor).Last + 1
              else
                 raise Program_Error),
          ...
          when F_Originate_Timestamp =>
             (if
                 ...
                 and then (RFLX_Types.U64 (Ctx.Cursors (F_Tag).Value.Tag_Value) = RFLX_Types.U64 (To_Base (RFLX.ICMP.Timestamp_Msg))
                           or RFLX_Types.U64 (Ctx.Cursors (F_Tag).Value.Tag_Value) = RFLX_Types.U64 (To_Base (RFLX.ICMP.Timestamp_Reply)))
              then
                 Ctx.Cursors (Ctx.Cursors (Fld).Predecessor).Last + 1
              else
          ...
          when F_Data =>
             (if
              ...
              then
                 ...
                 Ctx.Cursors (Fld).Predecessor = F_Sequence_Number
                 and then (RFLX_Types.U64 (Ctx.Cursors (F_Tag).Value.Tag_Value) = RFLX_Types.U64 (To_Base (RFLX.ICMP.Echo_Reply))
                           or RFLX_Types.U64 (Ctx.Cursors (F_Tag).Value.Tag_Value) = RFLX_Types.U64 (To_Base (RFLX.ICMP.Echo_Request)))
              then
                 Ctx.Cursors (Ctx.Cursors (Fld).Predecessor).Last + 1
              elsif
   ...
   function Valid_Predecessor (Ctx : Context; Fld : Virtual_Field) return Boolean is
     ((Fld = F_Initial
      ...
      or (Fld = F_Code_Destination_Unreachable
          and ((Valid (Ctx.Cursors (F_Tag))
                and Ctx.Cursors (Fld).Predecessor = F_Tag)))
      or (Fld = F_Code_Redirect
          and ((Valid (Ctx.Cursors (F_Tag))
                and Ctx.Cursors (Fld).Predecessor = F_Tag)))
      or (Fld = F_Code_Time_Exceeded
          and ((Valid (Ctx.Cursors (F_Tag))
                and Ctx.Cursors (Fld).Predecessor = F_Tag)))
      or (Fld = F_Code_Zero
          and ((Valid (Ctx.Cursors (F_Tag))
                and Ctx.Cursors (Fld).Predecessor = F_Tag)))
      or (Fld = F_Checksum
          and ((Valid (Ctx.Cursors (F_Code_Destination_Unreachable))
      ...
      or (Fld = F_Gateway_Internet_Address
      ...
      or (Fld = F_Sequence_Number
          and ((Valid (Ctx.Cursors (F_Identifier))
                and Ctx.Cursors (Fld).Predecessor = F_Identifier)))
      or (Fld = F_Unused_24
          and ((Valid (Ctx.Cursors (F_Pointer))
                and Ctx.Cursors (Fld).Predecessor = F_Pointer)))
      or (Fld = F_Originate_Timestamp
          and ((Valid (Ctx.Cursors (F_Sequence_Number))
                and Ctx.Cursors (Fld).Predecessor = F_Sequence_Number)))
      or (Fld = F_Data
          and ((Valid (Ctx.Cursors (F_Gateway_Internet_Address))
      ...
      or (Fld = F_Receive_Timestamp
          and ((Valid (Ctx.Cursors (F_Originate_Timestamp))
                and Ctx.Cursors (Fld).Predecessor = F_Originate_Timestamp)))
      or (Fld = F_Transmit_Timestamp
          and ((Valid (Ctx.Cursors (F_Receive_Timestamp))
