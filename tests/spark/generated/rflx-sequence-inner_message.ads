...
is
   ...
   procedure To_Context (Struct : Structure; Ctx : in out Context) with
     Pre =>
       ...
       and then Has_Buffer (Ctx)
       and then Valid_Next (Ctx, F_Length)
       and then Available_Space (Ctx, F_Length) >= 2048,
     Post =>
       Has_Buffer (Ctx);
      ...
      and then (if
                ...
                then
                   ...
                   and then (if
                                Structural_Valid (Cursors (F_Payload))
                             then
                                Cursors (F_Payload).Last - Cursors (F_Payload).First + 1 = RFLX_Types.Bit_Length (Cursors (F_Length).Value.Length_Value) * 8
                                and then Cursors (F_Payload).Predecessor = F_Length
