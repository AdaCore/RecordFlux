...
is
   ...
   function Field_Condition (Ctx : Context; Val : Field_Dependent_Value; Size : Types.Bit_Length := 0) return Boolean is
     ((case Val.Fld is
          when F_Initial | F_Destination | F_Source =>
             True,
          when F_Type_Length_TPID =>
             Types.U64 (Val.Type_Length_TPID_Value) = 16#8100#
             or Types.U64 (Val.Type_Length_TPID_Value) /= 16#8100#,
          when F_TPID | F_TCI =>
             True,
          when F_Type_Length =>
             ...
             (case Fld is
                 when F_Payload =>
                    (if
                     ...
                     then
                        Types.Bit_Length (Ctx.Last) - Types.Bit_Length (Ctx.Cursors (F_Type_Length).Last)
                     else
                        Types.Unreachable_Bit_Length),
                 when others =>
                    Types.Unreachable_Bit_Length),
