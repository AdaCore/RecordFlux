...
is
   ...
   function Successor (Ctx : Context; Fld : Field) return Virtual_Field is
    ...
    with
     Pre =>
       ...
       and RFLX.Sequence.Inner_Message.Structural_Valid (Ctx, Fld)
       and RFLX.Sequence.Inner_Message.Valid_Predecessor (Ctx, Fld);
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       RFLX.Sequence.Inner_Message.Has_Buffer (Ctx)
       and then RFLX.Sequence.Inner_Message.Valid_Next (Ctx, Fld)
       and then RFLX.Sequence.Inner_Message.Valid_Value (Fld, Val)
       and then RFLX.Sequence.Inner_Message.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.Sequence.Inner_Message.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.Sequence.Inner_Message.Composite_Field (Fld)
                 then
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.Sequence.Inner_Message.Valid_Value (Fld, Val)
       and then RFLX.Sequence.Inner_Message.Valid_Size (Ctx, Fld, RFLX.Sequence.Inner_Message.Field_Size (Ctx, Fld))
       and then RFLX.Sequence.Inner_Message.Available_Space (Ctx, Fld) >= RFLX.Sequence.Inner_Message.Field_Size (Ctx, Fld)
       and then RFLX.Sequence.Inner_Message.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.Sequence.Inner_Message.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
       and Invalid_Successor (Ctx, Fld)
   ...
   procedure Initialize_Payload_Private (Ctx : in out Context; Length : RFLX_Types.Length) with
     Pre =>
       ...
       and then RFLX.Sequence.Inner_Message.Field_First (Ctx, RFLX.Sequence.Inner_Message.F_Payload) mod RFLX_Types.Byte'Size = 1,
     Post =>
       Has_Buffer (Ctx)
   ...
   begin
      ...
      Set_Payload (Ctx, Struct.Payload (Struct.Payload'First .. Struct.Payload'First + RFLX_Types.Index (RFLX_Types.To_Length (RFLX_Types.Bit_Length (Struct.Length) * 8) + 1) - 2));
   end To_Context;
