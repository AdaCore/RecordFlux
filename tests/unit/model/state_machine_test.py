...
def test_identifier_normalization(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(StateMachine, "_check_identifiers", lambda _: None)
    assert str(
        StateMachine(
            "P::S",
            [
...
def test_unchecked_state_machine_checked() -> None:
    unchecked = UncheckedStateMachine(
        ID("P::S"),
        [
            State(
                "A",
...
def test_unchecked_state_machine_checked_error(
    unchecked: UncheckedStateMachine,
    expected: str,
) -> None:
