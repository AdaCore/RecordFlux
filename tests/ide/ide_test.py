...
def test_multiple_errors() -> None:
    ...
    assert_error(
        ...
        [
            ...
            f'{path}:260:9: model: error: field "F3" not congruent with overlaid field "F1"',
            f"{path}:260:9: model: info: unsatisfied \"F1'First = Message'First\"",
            f"{path}:262:10: model: info: unsatisfied \"F1'Last = (Message'First + 8) - 1\"",
            f"{path}:264:13: model: info: unsatisfied \"(F1'First + 16) - 1 = F1'Last\"",
