...
generic
   ...
   with function Valid (Element : {prefix}RFLX_Types.S63) return Boolean;
   with function To_Actual (Element : {prefix}RFLX_Types.S63) return Element_Type;
   with function To_S63 (Element : Element_Type) return {prefix}RFLX_Types.S63;
...
private
   ...
   type Context (Buffer_First, Buffer_Last : RFLX_Types.Index := RFLX_Types.Index'First; First : RFLX_Types.Bit_Index := RFLX_Types.Bit_Index'First; Last : RFLX_Types.Bit_Length := RFLX_Types.Bit_Length'First) is
      record
         ...
         First_Element : {prefix}RFLX_Types.S63 := {prefix}RFLX_Types.S63'First;
         Next_Element  : {prefix}RFLX_Types.S63 := {prefix}RFLX_Types.S63'First;
      end record with
     Dynamic_Predicate =>
       ((if Buffer /= null then
