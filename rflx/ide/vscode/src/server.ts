    ...
    return await window.withProgress(
        {
            ...
            title:
                "RecordFlux language server installation ([details](" +
                Uri.file(installLogPath) +
                "))",
            cancellable: true,
        },
        (progress, token) => {
            const promise = new Promise<boolean>((resolve) => {
                ...
                const process = spawn(workspaceSetting.interpreter[0], [
                    "-m",
                    "pip",
                    "install",
                    "RecordFlux",
                ...
                process.on("close", async (code, _signal) => {
                    ...
                    window.showErrorMessage(
                        "Failed to install the server ([details](" +
                            Uri.file(installLogPath) +
                            "))\n"
                    );
