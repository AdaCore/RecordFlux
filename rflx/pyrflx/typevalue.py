...
class TypeValue(Base):
    ...
    def _raise_initialized(self) -> None:
        if not self.initialized:
            e = PyRFLXError()
            e.push_msg(f"value {self.identifier} not initialized")
            raise e
    ...
    @abstractmethod
    def parse(self, value: Bitstring | bytes, check: bool = True) -> None:
        raise NotImplementedError
    ...
    def construct(
        ...
        e = PyRFLXError()
        e.push_msg("cannot construct unknown type: " + type(vtype).__name__)
        raise e
...
class IntegerValue(ScalarValue):
    ...
    def assign(self, value: int, check: bool = True) -> None:
        if check and (
            ...
            e = PyRFLXError()
            e.push_msg(f"value {value} not in type range {self._first} .. {self._last}")
            raise e
        self._value = value
...
class EnumValue(ScalarValue):
    ...
    def assign(self, value: str, check: bool = True) -> None:
        ...
        if Literal(prefixed_value) not in self.literals:
            e = PyRFLXError()
            e.push_msg(f"{value} is not a valid enum value")
            raise e
        r = (
            (
    ...
    def parse(self, value: Bitstring | bytes, _check: bool = True) -> None:
        if isinstance(value, bytes):
            value = Bitstring.from_bytes(value)
        ...
        if value_as_number not in self.literals.values():
            if self._type.always_valid:
                ...
                e = PyRFLXError()
                e.push_msg(f"Number {value_as_number.value} is not a valid enum value")
                raise e
        else:
...
class CompositeValue(TypeValue):
    def __init__(self, vtype: Composite) -> None:
        self._expected_size: Expr | None = None
        super().__init__(vtype)
    ...
    def _check_size_of_assigned_value(
        self,
        value: bytes | Bitstring | abc.Sequence[TypeValue],
    ) -> None:
        if isinstance(value, bytes):
...
class OpaqueValue(CompositeValue):
    _value: bytes | None
    _nested_message: MessageValue | None = None

    def __init__(self, vtype: Opaque) -> None:
        super().__init__(vtype)
        self._refinement_message: MessageValue | None = None
    ...
    @property
    def nested_message(self) -> MessageValue | None:
...
class MessageValue(TypeValue):
    ...
    def add_parameters(self, parameters: abc.Mapping[str, bool | int | str]) -> None:
        ...
        if expected - added:
            ...
            e = PyRFLXError()
            e.push_msg(f"missing parameter values: {message}")
            raise e

        if added - expected:
            message = ", ".join(added - expected)
            e = PyRFLXError()
            e.push_msg(f"unexpected parameter values: {message}")
            raise e
        ...
        for name, value in parameters.items():
            if isinstance(value, bool):
                ...
                e = PyRFLXError()  # type: ignore[unreachable]
                e.push_msg(
    ...
    def _next_link(self, source_field_name: str) -> Link | None:
    ...
    def _get_size(self, fld: str) -> Number | None:
    ...
    def _get_first(self, fld: str) -> Number | None:
        for l in self._type.incoming(Field(fld)):
    ...
    def _set_unchecked(
        self,
        field_name: str,
        value: bytes | int | str | abc.Sequence[TypeValue],
    ) -> None:
    ...
    def _set_checked(
        self,
        field_name: str,
        value: bytes | int | str | abc.Sequence[TypeValue] | Bitstring,
        ...
        if field_name in self.accessible_fields:
            ...
            try:
                ...
                new_exception = PyRFLXError()
                new_exception.push_msg(f"cannot set value for field {field_name}")
                new_exception.extend(e.entries)
                raise new_exception from e
        else:
            error = PyRFLXError()
            error.push_msg(f"cannot access field {field_name}")
            raise error
    ...
    def _calculate_checksum(self, checksum: MessageValue.Checksum) -> int:
        ...
        arguments: dict[str, str | int | bytes | tuple[int, int] | list[int]] = {}
        for expr_tuple in checksum.parameters:
    ...
    def get(self, field_name: str) -> ValueType:
        if field_name not in self.valid_fields:
            if field_name not in self.fields:
                e = PyRFLXError()
                e.push_msg(f'"{field_name}" is not a field of this message')
                raise e
            e = PyRFLXError()
            e.push_msg(f'"{field_name}" is not set')
            raise e
        field = self._fields[field_name]
    ...
    def bytestring(self) -> bytes:
        if not self._skip_verification and not self.valid_message:
            e = PyRFLXError()
            e.push_msg(f"cannot create bytestring of invalid message: {self.identifier}")
            raise e
    ...
    class Checksum:
        def __init__(self, field_name: str, parameters: abc.Sequence[Expr]):
            ...
            self.function: ChecksumFunction | None = None
    ...
    class Field(Base):
        def __init__(  # noqa: PLR0913
            ...
            name: str = "",
            name_variable: Variable | None = None,
            name_first: First | None = None,
            name_last: Last | None = None,
            name_size: Size | None = None,
        ):
    ...
    @dataclass
    class State:
        fields: abc.Mapping[str, MessageValue.Field] | None = None
        checksums: abc.Mapping[str, MessageValue.Checksum] | None = None
