...
def iana_to_rflx(xml_input: TextIO, always_valid: bool, output_file: Optional[Path] = None) -> None:
    ...
    for registry in root.findall(root.tag):
        ...
        if enum_type is not None and len(enum_type.enum_literals) > 0:
            enum_types.append(enum_type)

    resolve_duplicate_literals(enum_types)
    write_rflx_specification(
        output_file, enum_types, package_name, registry_title, registry_last_updated
    )
...
def _convert_registry_to_enum_type(
    ...
    return EnumType(
        registry_title, list(enum_literals.values()), enum_literal_highest_bit_length, always_valid
    )
