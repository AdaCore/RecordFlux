...
def write_registry(
    ...
    normalized_records, highest_bit_length = _normalize_records(records, registry_title, duplicates)
    ...
    file.write(")\n")
    file.write(f"{'':<3}with Size => {highest_bit_length}")
    if always_valid and len(normalized_records) < 2 ** highest_bit_length:
        file.write(", Always_Valid;")
    else:
        assert len(normalized_records) <= 2 ** highest_bit_length
        file.write(";")
    file.write("\n\n")
